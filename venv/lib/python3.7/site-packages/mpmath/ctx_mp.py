...
class MPContext(BaseMPContext, StandardBaseContext):
    ...
    def init_builtins(ctx):
        ...
        eps = ctx.constant(
            lambda prec, rnd: (0, MPZ_ONE, 1 - prec, 1),
        ...
        ctx.fib = ctx.fibonacci = ctx._wrap_libmp_function(
            libmp.mpf_fibonacci, libmp.mpc_fibonacci
        )
        ...
        ctx.fac = ctx.factorial = ctx._wrap_libmp_function(
            libmp.mpf_factorial, libmp.mpc_factorial
        )
        ctx.gamma_old = ctx._wrap_libmp_function(
            libmp.mpf_gamma_old, libmp.mpc_gamma_old
        )
        ctx.fac_old = ctx.factorial_old = ctx._wrap_libmp_function(
            libmp.mpf_factorial_old, libmp.mpc_factorial_old
        )
    ...
    def __str__(ctx):
        lines = [
            ...
            ("  mp.trap_complex = %s" % ctx.trap_complex).ljust(30)
            + "[default: False]",
        ]
        return "\n".join(lines)
...
maxterms, or set zeroprec."""
        ...
        **Examples**
        ...
        try:
            if hasattr(x, "_mpf_"):
                ...
                if hasattr(y, "_mpc_"):
                    return ctx.make_mpc(
                        mpc_sub((x._mpf_, fzero), y._mpc_, prec, rounding)
                    )
            if hasattr(x, "_mpc_"):
                if hasattr(y, "_mpf_"):
    ...
    def fraction(ctx, p, q):
        ...
        """
        return ctx.constant(
            lambda prec, rnd: from_rational(p, q, prec, rnd), "%s/%s" % (p, q)
        )
