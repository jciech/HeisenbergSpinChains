    ...
    def transform_nodes(self, nodes, a, b, verbose=False):
        ...
        if ctx.isinf(a) or ctx.isinf(b):
            if (a, b) == (ctx.ninf, ctx.inf):
                ...
                return [
                    (x, -w) for (x, w) in self.transform_nodes(nodes, b, a, verbose)
                ]
            else:
                raise NotImplementedError
        else:
    ...
    def summation(self, f, points, prec, epsilon, max_degree, verbose=False):
        ...
        for i in xrange(len(points) - 1):
            ...
            for degree in xrange(1, max_degree + 1):
                ...
                if verbose:
                    print(
                        "Integrating from %s to %s (degree %s of %s)"
            ...
            if dim == 1:
                ...
                v, err = rule.summation(
                    lambda x: rule.summation(
                        lambda y: f(x, y), points[1], prec, epsilon, m
                    )[0],
                    points[0],
                    prec,
                    epsilon,
                ...
                v, err = rule.summation(
                    lambda x: rule.summation(
                        lambda y: rule.summation(
                            lambda z: f(x, y, z), points[2], prec, epsilon, m
                        )[0],
                        points[1],
                        prec,
                        epsilon,
        ...
        **Fast decay**
        ...
        if a == ctx.ninf:
            if zeros:
                ...
                return ctx.quadosc(
                    lambda x: f(-x), [-b, -a], omega=omega, period=period
                )
        if b != ctx.inf:
