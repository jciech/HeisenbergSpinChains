...
def aux_M_Fp(ctx, xA, xeps4, a, xB1, xL):
    ...
    m = 3 * xL - 3
    aux3 = (ctx.loggamma(m + 1) - ctx.loggamma(m / 3.0 + 2)) / 2 - ctx.loggamma(
        (m + 1) / 2.0
    )
...
def Rzeta_simul(ctx, s, der=0):
    ...
    for n in range(1, L):
        ...
        for k in range(0, 3 * n // 2 + 1):
            ...
            if m != 0:
                ...
                for r in range(0, k):
                    add = xd[0, n, r] * (
                        ctx.mpf("1.0") * ctx.fac(2 * k - 2 * r) / ctx.fac(k - r)
                    )
                    xd[0, n, k] -= ((-1) ** (k - r)) * add
        xd[0, n, -2] = 0
        xd[0, n, -1] = 0
    ...
    for mu in range(0, der + 1):
        for k in range(0, L):
            ...
            for ell in range(0, 3 * k // 2 + 1):
                xfortcoef[mu, k, ell] = (
                    xd[mu, k, ell] * Fp[3 * k - 2 * ell] / pipowers[2 * k - ell]
                )
    ...
    for mu in range(0, der + 1):
        for k in range(0, L):
            ...
            for ell in range(0, 3 * k // 2 + 1):
                yfortcoef[mu, k, ell] = (
                    yd[mu, k, ell] * Fp[3 * k - 2 * ell] / pipowers[2 * k - ell]
                )
    ...
    **Definition**
    ...
    for n in range(1, L):
        ...
        for k in range(0, 3 * n // 2 + 1):
            ...
            if m != 0:
                ...
                for r in range(0, k):
                    add = d[0, n, r] * (
                        ctx.one * ctx.fac(2 * k - 2 * r) / ctx.fac(k - r)
                    )
                    d[0, n, k] -= ((-1) ** (k - r)) * add
        d[0, n, -2] = 0
        d[0, n, -1] = 0
    ...
    for mu in derivatives:
        for k in range(0, L):
            ...
            for ell in range(0, 3 * k // 2 + 1):
                fortcoef[mu, k, ell] = (
                    d[mu, k, ell] * Fp[3 * k - 2 * ell] / pipowers[2 * k - ell]
                )
    ...
    for chi in derivatives:
        for k in range(0, L):
            ...
            for ell in range(0, 3 * k // 2 + 1):
                ...
                for mu in range(0, chi + 1):
                    tcoefter = (
                        ctx.binomial(chi, mu) * la ** mu * fortcoef[chi - mu, k, ell]
                    )
...
def z_offline(ctx, w, k=0):
    ...
    if ctx._re(s2) >= 0:
        ...
        M2 = (
            4
            * ctx._im(s2)
            * (2 * ctx.pi) ** (ctx._re(s2) - 1)
            * abs(1 - s2) ** (0.5 - ctx._re(s2))
        )
    ...
    if k == 1:
        zv = (
            j * exptheta * (xrz[1] + xrz[0] * ps1)
            - j * (yrz[1] + yrz[0] * ps1) / exptheta
        )
    if k == 2:
        zv = exptheta * (
            -2 * xrz[1] * ps1 - xrz[0] * ps1 ** 2 - xrz[2] + j * xrz[0] * ps2
        )
        zv = (
            zv
            + (-2 * yrz[1] * ps1 - yrz[0] * ps1 ** 2 - yrz[2] - j * yrz[0] * ps2)
            / exptheta
        )
    if k == 3:
        zv1 = (
