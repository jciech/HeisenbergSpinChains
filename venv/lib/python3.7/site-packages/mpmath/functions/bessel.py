    ...
    if derivative:
        ...
        if ctx.isint(d) and d >= 0:
            ...
            try:
                ctx.prec += 15
                v = ctx.fsum(
                    (-1) ** k * ctx.binomial(d, k) * ctx.besselj(2 * k + n - d, z)
                    for k in range(d + 1)
                )
            finally:
                ctx.prec = orig
...
def bessely(ctx, n, z, derivative=0, **kwargs):
    ...
    return (
        ctx.besselj(n, z, derivative, **kwargs) * cos
        - ctx.besselj(-n, z, derivative, **kwargs)
    ) / sin
...
def _airy_zero(ctx, which, k, derivative, complex=False):
    ...
    if which == 0:
        if derivative:
            return ctx.findroot(
                lambda z: ctx.airyai(z, 1), -U(3 * ctx.pi * (4 * k - 3) / 8)
            )
        return ctx.findroot(ctx.airyai, -T(3 * ctx.pi * (4 * k - 1) / 8))
    if which == 1 and complex == False:
        if derivative:
            return ctx.findroot(
                lambda z: ctx.airybi(z, 1), -U(3 * ctx.pi * (4 * k - 1) / 8)
            )
...
def mcmahon(ctx, kind, prime, v, m):
    ...
    if not prime:
        ...
        s5 = (
            -64
            * (u - 1)
            * (6949 * u ** 3 - 153855 * u ** 2 + 1585743 * u - 6277237)
            / (105 * (8 * b) ** 7)
        )
    if prime:
        s1 = b
