...
small_factorial_cache = [
    from_int(ifac(n)) for n in range(SMALL_FACTORIAL_CACHE_SIZE + 1)
]
...
def zeta_array(N, prec):
    ...
    for n in xrange(5, N + 1, 4):
        ...
        for k in xrange(1, 2 * U + 1):
            s += (
                (-1) ** k * 2 * k * zeta_values[2 * k] * zeta_values[4 * U + 2 - 2 * k]
            ) >> wp
...
def mpf_gamma(x, prec, rnd="d", type=0):
    ...
    if exp >= -1:
        ...
        if n < 100 or gamma_size < 10 * wp:
            if sign:
                ...
                if type == 3:
                    return mpf_log(
                        mpf_shift(from_rational(w, abs(f), prec, rnd), -wp + n + 1),
                        prec,
                        rnd,
                    )
            elif n == 0:
    ...
    if type == 3 and not sign:
        ...
        if cancellation > 10:
            ...
            if xsub2mag < -wp:
                return mpf_mul(
                    mpf_sub(fone, mpf_euler(wp)), mpf_sub(x, ftwo), prec, rnd
                )
...
def mpc_gamma(z, prec, rnd="d", type=0):
    ...
    if not yfinal:
        ...
        if absn < n_for_stirling:
            ...
            for k in xrange(d):
                rre, rim = (
                    ((afix * rre - bfix * rim) >> wp),
                    ((afix * rim + bfix * rre) >> wp),
                )
                afix += one
