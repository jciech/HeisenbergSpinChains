...
def make_hyp_summator(key):
    ...
    if have_complex:
        ...
        if multiplier:
            if have_complex_arg:
                add(
                    "    PRE, PIM = (mul*(PRE*ZRE-PIM*ZIM))//div,"
                    " (mul*(PIM*ZRE+PRE*ZIM))//div"
                )
                add("    PRE >>= wp")
                add("    PIM >>= wp")
            else:
        ...
        for i in acomplex:
            add(
                "    PRE, PIM = PRE*ACRE_#-PIM*ACIM_#, PIM*ACRE_#+PRE*ACIM_#".replace(
                    "#", str(i)
                )
            )
            add("    PRE >>= wp")
    ...
    add("    if n > MAX:")
    add(
        "        raise NoConvergence('Hypergeometric series converges too slowly. Try"
        " increasing maxterms.')"
    )
    ...
    source = (
        "def %s(coeffs, z, prec, wp, epsshift, magnitude_check, **kwargs):\n" % fname
        + source
    )
...
def complex_ei_taylor(zre, zim, prec):
    ...
    while _abs(tre) + _abs(tim) > 5:
        tre, tim = (
            ((tre * zre - tim * zim) // k) >> prec,
            ((tre * zim + tim * zre) // k) >> prec,
        )
        sre += tre // k
        sim += tim // k
        k += 1
...
def complex_ei_asymptotic(zre, zim, prec):
    ...
    while _abs(tre) + _abs(tim) > 1000:
        # print tre, tim
        tre, tim = (
            ((tre * xre - tim * xim) * k) >> prec,
            ((tre * xim + tim * xre) * k) >> prec,
        )
        sre += tre
        sim += tim
        k += 1
...
def mpf_expint(n, x, prec, rnd=round_fast, gamma=False):
    ...
    if n == 0 or 2 * nmag - xmag < -wp:
        ...
        if not can_use_asymptotic_series:
            ...
            siz = (
                -n * nmag + (m + n) * bitcount(abs(m + n)) - m * xmag - (144 * m // 100)
            )
            tol = -wp - 10
    ...
    if have_imag:
        ...
        if gamma:
            ...
            im = mpf_div(
                mpf_mul(mpf_pi(wp), mpf_pow_int(negx, n_orig - 1, wp), wp), M, prec, rnd
            )
        return re, im
    else:
        return re, None
