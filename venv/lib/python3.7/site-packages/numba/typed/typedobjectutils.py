...
def _get_incref_decref(context, module, datamodel, container_type):
    ...
    incref_fn = module.get_or_insert_function(
        refct_fnty, name=".numba_{}_incref${}".format(container_type, fe_type),
    )

    builder = ir.IRBuilder(incref_fn.append_basic_block())
    context.nrt.incref(
        builder, fe_type, datamodel.load_from_data_pointer(builder, incref_fn.args[0]),
    )
    builder.ret_void()

    decref_fn = module.get_or_insert_function(
        refct_fnty, name=".numba_{}_decref${}".format(container_type, fe_type),
    )
    builder = ir.IRBuilder(decref_fn.append_basic_block())
    context.nrt.decref(
        builder, fe_type, datamodel.load_from_data_pointer(builder, decref_fn.args[0]),
    )
    builder.ret_void()
...
def _get_equal(context, module, datamodel, container_type):
    ...
    wrapfn = module.get_or_insert_function(
        wrapfnty, name=".numba_{}_item_equal.wrap${}".format(container_type, fe_type)
    )
    ...
    equal_fn = module.get_or_insert_function(
        equal_fnty, name=".numba_{}_item_equal${}".format(container_type, fe_type),
    )
