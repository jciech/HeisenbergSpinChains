...
def _list_codegen_set_method_table(context, builder, lp, itemty):
    vtablety = ir.LiteralStructType(
        [ll_voidptr_type, ll_voidptr_type,]  # item incref  # item decref
    )
    ...
    if dm_item.contains_nrt_meminfo():
        ...
        builder.store(
            builder.bitcast(item_incref, item_incref_ptr.type.pointee), item_incref_ptr,
        )
        builder.store(
            builder.bitcast(item_decref, item_decref_ptr.type.pointee), item_decref_ptr,
        )
...
def _add_meminfo(context, builder, lstruct):
    ...
    meminfo = context.nrt.meminfo_alloc_dtor(
        builder, context.get_constant(types.uintp, alloc_size), dtor,
    )
...
def _list_new_codegen(context, builder, itemty, new_size, error_handler):
    fnty = ir.FunctionType(
        ll_status, [ll_list_type.as_pointer(), ll_ssize_t, ll_ssize_t],
    )
...
def _list_new(typingctx, itemty, allocated):
    ...
    def codegen(context, builder, sig, args):
        ...
        return _list_new_codegen(
            context, builder, itemty.instance_type, args[1], error_handler,
        )
...
def _list_allocated(typingctx, l):
    ...
    def codegen(context, builder, sig, args):
        fnty = ir.FunctionType(ll_ssize_t, [ll_list_type],)
        fn = builder.module.get_or_insert_function(fnty, name="numba_list_allocated")
        [l] = args
        [tl] = sig.args
        lp = _container_get_data(context, builder, tl, l)
        n = builder.call(fn, [lp])
...
def _list_is_mutable(typingctx, l):
    ...
    def codegen(context, builder, sig, args):
        ...
        fn = builder.module.get_or_insert_function(fnty, name="numba_list_is_mutable")
        [l] = args
        [tl] = sig.args
        lp = _container_get_data(context, builder, tl, l)
        n = builder.call(fn, [lp])
...
def _list_getitem_pop_helper(typingctx, l, index, op):
    ...
    resty = types.Tuple(
        [types.int32, types.Optional(l.item_type if IS_NOT_NONE else types.int64)]
    )
    sig = resty(l, index)
...
def _list_setitem(typingctx, l, index, item):
    ...
    def codegen(context, builder, sig, args):
        ...
        fn = builder.module.get_or_insert_function(fnty, name="numba_list_setitem")
...
def _list_delete_slice(typingctx, l, start, stop, step):
    ...
    def codegen(context, builder, sig, args):
        fnty = ir.FunctionType(
            ll_status, [ll_list_type, ll_ssize_t, ll_ssize_t, ll_ssize_t],
        )
        [l, start, stop, step] = args
        [tl, tstart, tstop, tstep] = sig.args
        fn = builder.module.get_or_insert_function(fnty, name="numba_list_delete_slice")
...
def impl_iterator_iternext(context, builder, sig, args, result):
    ...
    iternext_fnty = ir.FunctionType(
        ll_status, [ll_listiter_type, ll_bytes.as_pointer()]
    )
    iternext = builder.module.get_or_insert_function(
        iternext_fnty, name="numba_list_iter_next",
    )
    ...
    with builder.if_then(is_valid, likely=True):
        ...
        item_ptr = builder.bitcast(
            builder.load(item_raw_ptr), dm_item.get_data_type().as_pointer(),
        )
