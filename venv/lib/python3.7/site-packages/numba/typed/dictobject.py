...
def _dict_new_minsize(typingctx, keyty, valty):
    ...
    def codegen(context, builder, sig, args):
        fnty = ir.FunctionType(
            ll_status, [ll_dict_type.as_pointer(), ll_ssize_t, ll_ssize_t],
        )
        ...
        _raise_if_error(
            context, builder, status, msg="Failed to allocate dictionary",
        )
...
def _dict_set_method_table(typingctx, dp, keyty, valty):
    ...
    def codegen(context, builder, sig, args):
        ...
        setmethod_fn = ir.Function(
            builder.module, setmethod_fnty, name="numba_dict_set_method_table",
        )
        dp = args[0]
        ...
        if dm_key.contains_nrt_meminfo():
            ...
            builder.store(
                builder.bitcast(equal, key_equal_ptr.type.pointee), key_equal_ptr,
            )
            builder.store(
...
def _dict_insert(typingctx, d, key, hashval, val):
    ...
    def codegen(context, builder, sig, args):
        fnty = ir.FunctionType(
            ll_status, [ll_dict_type, ll_bytes, ll_hash, ll_bytes, ll_bytes],
        )
...
def _dict_lookup(typingctx, d, key, hashval):
    ...
    def codegen(context, builder, sig, args):
        ...
        ix = builder.call(
            fn,
            [dp, _as_bytes(builder, ptr_key), hashval, _as_bytes(builder, ptr_val),],
        )
...
def _dict_popitem(typingctx, d):
    ...
    def codegen(context, builder, sig, args):
        ...
        dp = _container_get_data(context, builder, td, d)
        status = builder.call(
            fn, [dp, _as_bytes(builder, ptr_key), _as_bytes(builder, ptr_val),],
        )
...
def impl_iterator_iternext(context, builder, sig, args, result):
    ...
    iternext = builder.module.get_or_insert_function(
        iternext_fnty, name="numba_dict_iter_next",
    )
    ...
    with builder.if_then(is_valid):
        ...
        key_ptr = builder.bitcast(
            builder.load(key_raw_ptr), dm_key.get_data_type().as_pointer(),
        )
        val_ptr = builder.bitcast(
            builder.load(val_raw_ptr), dm_val.get_data_type().as_pointer(),
        )
