...
class _ModuleCompiler(object):
    ...
    method_def_ty = lc.Type.struct(
        (lt._int8_star, lt._void_star, lt._int32, lt._int8_star)
    )
    ...
    def emit_header(self, output):
        ...
        with open(fname + ".h", "w") as fout:
            ...
            for export_entry in self.export_entries:
                ...
                args = ", ".join(
                    self.emit_type(argtype) for argtype in export_entry.signature.args
                )
    ...
    def _emit_method_array(self, llvm_module):
        ...
        method_array = llvm_module.add_global_variable(
            method_array_init.type, ".module_methods"
        )
    ...
    def _emit_environment_array(self, llvm_module, builder, pyapi):
        ...
        gv = self.context.insert_unique_const(
            llvm_module, ".module_environments", env_defs_init
        )
    ...
    def _emit_envgvs_array(self, llvm_module, builder, pyapi):
        ...
        gv = self.context.insert_unique_const(
            llvm_module, ".module_envgvs", env_setters_init
        )
        return gv.gep([ZERO, ZERO])

    def _emit_module_init_code(
        ...
        if self.external_init_function:
            fnty = ir.FunctionType(
                lt._int32,
                [modobj.type, self.method_def_ptr, self.env_def_ptr, envgv_array.type],
            )
...
class ModuleCompiler(_ModuleCompiler):
    ...
    traverseproc_ty = _ptr_fun(
        lt._int8, lt._pyobject_head_p, visitproc_ty, lt._void_star
    )
    ...
    #:   PyObject* m_copy;
    #: } PyModuleDef_Base;
    module_def_base_ty = lc.Type.struct(
        (lt._pyobject_head, m_init_ty, lt._llvm_py_ssize_t, lt._pyobject_head_p)
    )
    ...
    def module_create_definition(self):
        ...
        """
        signature = lc.Type.function(
            lt._pyobject_head_p, (lc.Type.pointer(self.module_def_ty), lt._int32)
        )
    ...
    def _emit_python_wrapper(self, llvm_module):
        ...
        mod_def_base = llvm_module.add_global_variable(
            mod_def_base_init.type, ".module_def_base"
        )
        ...
        mod = builder.call(
            create_module_fn, (mod_def, lc.Constant.int(lt._int32, sys.api_version))
        )
        ...
        ret = self._emit_module_init_code(
            llvm_module, builder, mod, method_array, env_array, envgv_array
        )
        if ret is not None:
