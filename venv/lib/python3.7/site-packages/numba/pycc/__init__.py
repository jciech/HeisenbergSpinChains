...
def main(args=None):
    ...
    group.add_argument(
        "--llvm", action="store_true", help="Emit llvm instead of native code"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Emit additionally generated Python wrapper and "
            "extension module code in output"
        ),
    )
    parser.add_argument(
        "-d", "--debug", action="store_true", help="Print extra debug information"
    )
    ...
    logger.warn(
        "The 'pycc' script is DEPRECATED; please use the numba.pycc.CC API instead"
    )
    ...
    if args.header:
        print(
            "ERROR: pycc --header has been disabled in this release due to a known"
            " issue"
        )
        sys.exit(1)
