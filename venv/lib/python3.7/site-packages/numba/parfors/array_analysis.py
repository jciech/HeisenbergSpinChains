    ...
    def _get_or_set_rel(self, name, func_ir=None):
        ...
        if name in self.def_by:
            ...
            if isinstance(value, int) or (
                isinstance(value, tuple) and (value[0] != name or value[1] != 0)
            ):
                    ...
                    if ind >= 0:
                        ...
                        for obj in objs:
                            if obj in self.ref_by:
                                names += [
                                    x for (x, i) in self.ref_by[obj] if i == -offset
                                ]
                        if len(names) > 1:
...
class ArrayAnalysis(object):
    ...
    def run(self, blocks=None, equiv_set=None):
        ...
        if config.DEBUG_ARRAY_OPT >= 1:
            self.dump()
            print(
                "ArrayAnalysis post variable types: ", sorted(self.typemap.items()),
            )
        ...
        if isinstance(inst, ir.Assign):
            ...
            if isinstance(typ, types.ArrayCompatible) and typ.ndim == 0:
                ...
                if result:
                    ...
                    if len(result) > 2:
                        rhs = result[2]
                        inst.value = rhs
            elif isinstance(inst.value, ir.Var) or isinstance(inst.value, ir.Const):
                shape = inst.value
            ...
            if isinstance(typ, types.ArrayCompatible):
                if (
                    ...
                    and isinstance(self.typemap[shape.name], types.containers.Tuple)
                ):
                    pass
                elif (
                    shape is None
    ...
    def gen_literal_slice_part(
        self, arg_val, loc, scope, stmts, equiv_set, name="static_literal_slice_part",
    ):
    ...
    def gen_static_slice_size(self, lhs_rel, rhs_rel, loc, scope, stmts, equiv_set):
        the_var, *_ = self.gen_literal_slice_part(
            rhs_rel - lhs_rel, loc, scope, stmts, equiv_set, name="static_slice_size",
        )
        return the_var

    def gen_explicit_neg(
        self, arg, arg_rel, arg_typ, size_typ, loc, scope, dsize, stmts, equiv_set,
    ):
    ...
    def slice_size(self, index, dsize, equiv_set, scope, stmts):
        ...
        if not need_replacement:
            ...
            stmts.append(
                ir.Assign(
                    value=replacement_slice, target=replacement_slice_var, loc=loc,
                )
            )
        ...
        if config.DEBUG_ARRAY_OPT >= 2:
            print(
                "after rewriting negatives", "lhs_rel", lhs_rel, "rhs_rel", rhs_rel,
            )
        ...
        stmts.append(
            ir.Assign(value=post_wrap_size_val, target=post_wrap_size_var, loc=loc)
        )
        ...
        if replace_index:
            ...
            if len(index_var_list) > 1:
                ...
                replacement_build_tuple_var = ir.Var(
                    scope, mk_unique_var("replacement_build_tuple"), ind_shape[0].loc,
                )
    ...
    def _analyze_op_call(self, scope, equiv_set, expr):
        ...
        if isinstance(callee_def, (ir.Global, ir.FreeVar)) and is_namedtuple_class(
            callee_def.value
        ):
        ...
        if isinstance(callee_def, (ir.Global, ir.FreeVar)) and isinstance(
            ...
            return self._analyze_stencil(
                scope, equiv_set, callee_def.value, expr.loc, args, dict(expr.kws),
            )
    ...
    def _broadcast_assert_shapes(self, scope, equiv_set, loc, shapes, names):
        ...
        for i in range(max_dim):
            ...
            if sizes == []:
                ...
                size_names.append("1")
            asserts.append(
                self._call_assert_equiv(scope, loc, equiv_set, sizes, names=size_names)
            )
    ...
    def _make_assert_equiv(self, scope, loc, equiv_set, _args, names=None):
        ...
        if len(args) < 2:
            if config.DEBUG_ARRAY_OPT >= 2:
                print(
                    "Will not insert assert_equiv as args are known to be equivalent."
                )
            return []
