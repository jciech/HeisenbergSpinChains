    ...
    if nredvars > 0:
        ...
        for i in range(nredvars):
            ...
            glbl_np_empty = pfbdr.bind_global_function(
                ...
                args=(types.UniTuple(types.intp, redarrdim), types.DType(reddtype),),
            )
            ...
            num_threads_var = pfbdr.make_const_variable(
                cval=thread_count, typ=types.intp, name="num_threads",
            )
            ...
            if init_val is not None:
                if isinstance(redvar_typ, types.npytypes.Array):
                    ...
                    init_val_var = pfbdr.make_const_variable(
                        cval=init_val, typ=reddtype, name="init_val",
                    )
                    ...
                    redtoset = pfbdr.assign(
                        rhs=full_call, typ=redvar_typ, name="redtoset",
                    )
                else:
                    redtoset = pfbdr.make_const_variable(
                        cval=init_val, typ=reddtype, name="redtoset",
                    )
            else:
                redtoset = redvar
        ...
        for i in range(nredvars):
            ...
            if config.DEBUG_ARRAY_OPT_RUNTIME:
                ...
                lhs = pfbldr.make_const_variable(
                    cval=res_print_str, typ=strconsttyp, name="str_const",
                )
            ...
            for j in range(thread_count):
                ...
                oneelem = pfbdr.assign(
                    rhs=oneelemgetitem, typ=redvar_typ, name="redelem",
                )
        ...
        if len(diff) > 0:
            ...
            if config.DEBUG_ARRAY_OPT >= 1:
                print(
                    "Instruction",
                    inst,
                    " could not be hoisted because of a dependency.",
                )
        else:
...
def redarraytype_to_sig(redarraytyp):
    ...
    return types.npytypes.Array(
        redarraytyp.dtype, max(1, redarraytyp.ndim - 1), redarraytyp.layout
    )
...
def call_parallel_gufunc(
    ...
    get_num_threads = builder.module.get_or_insert_function(
        lc.Type.function(lc.Type.int(types.intp.bitwidth), []), name="get_num_threads"
    )
