...
class _Stat(object):
    ...
    def describe(self):
        ...
        return "supported = {supported} / {total} = {ratio:.2f}%".format(
            supported=self.supported, total=self.total, ratio=self.ratio,
        )

    def __repr__(self):
        return "{clsname}({describe})".format(
            clsname=self.__class__.__name__, describe=self.describe(),
        )
...
def list_modules_in_package(package, module_filters=_default_module_filters):
    ...
    package_walker = pkgutil.walk_packages(
        package.__path__, prefix, onerror=onerror_ignore,
    )
...
def main():
    ...
    parser.add_argument(
        "package", metavar="package", type=str, help="Package to inspect",
    )
    parser.add_argument(
        "--format",
        dest="format",
