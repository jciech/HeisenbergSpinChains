    ...
    **kws
    ...
    if argtypes is None and not sigutils.is_signature(func_or_sig):
            ...
            if config.ENABLE_CUDASIM:
                ...
                def autojitwrapper(func):
                    return FakeCUDAKernel(
                        func, device=device, fastmath=fastmath, debug=debug
                    )
        ...
        # func_or_sig is a function
            ...
            if config.ENABLE_CUDASIM:
                return FakeCUDAKernel(
                    func_or_sig, device=device, fastmath=fastmath, debug=debug
                )
            elif device:
                ...
                return AutoJitCUDAKernel(
                    func_or_sig, bind=bind, targetoptions=targetoptions
                )
        ...
        def kernel_jit(func):
            kernel = compile_kernel(
                func, argtypes, link=link, debug=debug, inline=inline, fastmath=fastmath
            )
        ...
        def device_jit(func):
            return compile_device(func, restype, argtypes, inline=inline, debug=debug)
