...
@require_context
def mapped_array(
    shape, dtype=np.float, strides=None, order="C", stream=0, portable=False, wc=False
):
...
def device_array_like(ary, stream=0):
    ...
    if ary.flags["C_CONTIGUOUS"] or ary.ndim <= 1:
        ...
        return device_array(shape=ary.shape, dtype=ary.dtype, order="F", stream=stream)
    ...
    return device_array(
        shape=ary.shape, dtype=ary.dtype, strides=strides, stream=stream
    )
