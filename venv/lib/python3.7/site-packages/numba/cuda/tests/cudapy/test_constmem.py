...
class TestCudaConstantMemory(CUDATestCase):
    def test_const_array(self):
        ...
        if not ENABLE_CUDASIM:
            self.assertIn(
                "ld.const.f64", jcuconst.ptx, "as we're adding to it, load as a double"
            )
    ...
    def test_const_array_2d(self):
        ...
        if not ENABLE_CUDASIM:
            self.assertIn("ld.const.u32", jcuconst2d.ptx, "load the ints as ints")
    ...
    def test_const_record_align(self):
        ...
        if not ENABLE_CUDASIM:
            self.assertIn(
                "ld.const.v4.u8", jcuconst.ptx, "load the first three bytes as a vector"
            )

            self.assertIn("ld.const.u32", jcuconst.ptx, "load the uint32 natively")

            self.assertIn("ld.const.u8", jcuconst.ptx, "load the last byte by itself")
