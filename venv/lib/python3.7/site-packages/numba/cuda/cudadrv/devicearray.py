...
class DeviceNDArrayBase(object):
    ...
    def to_host(self, stream=0):
        stream = self._default_stream(stream)
        warnings.warn("to_host() is deprecated and will be removed", DeprecationWarning)
    ...
    def split(self, section, stream=0):
        ...
        for i in range(nsect):
            ...
            yield DeviceNDArray(
                shape, strides, dtype=self.dtype, stream=stream, gpu_data=gpu_data
            )
...
class DeviceNDArray(DeviceNDArrayBase):
        ...
        if len(extents) == 1:
            ...
            if not arr.is_array:
                ...
                _driver.device_to_host(
                    dst=hostary, src=newdata, size=self._dummy.itemsize, stream=stream
                )
                return hostary[0]
            else:
                return cls(
    ...
    def _do_setitem(self, key, value, stream=0):
        ...
        for i, (l, r) in enumerate(zip(lhs.shape, rhs.shape)):
            if r != 1 and l != r:
                raise ValueError(
                    "Can't copy sequence with size %d to array axis %d with"
                    " dimension %d"
                    % (r, i, l)
                )
