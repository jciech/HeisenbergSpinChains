...
class CompilationUnit(object):
    ...
    def compile(self, **options):
        ...
        # compile
        c_opts = (c_char_p * len(opts))(*[c_char_p(x.encode("utf8")) for x in opts])
        ...
        if cc == mycc:
            ...
            if i == 0:
                ...
                raise NvvmSupportError(
                    "GPU compute capability %d.%d is not supported (requires >=%d.%d)"
                    % (mycc + cc)
                )
            else:
...
def llvm_to_ptx(llvmir, **opts):
    ...
    replacements = [
        ("declare i32 @___numba_cas_hack(i32*, i32, i32)", ir_numba_cas_hack),
        (
...
re_metadata_debuginfo = re.compile(
    r"\!{i32 \d, \!\"Debug Info Version\", i32 \d}".replace(" ", r"\s+")
)
...
def llvm39_to_34_ir(ir):
    ...
    for line in ir.splitlines():
        ...
        if (
            line.lstrip().startswith('tail call void asm sideeffect "// dbg')
        ...
        if "call " in line:
            ...
            if "@llvm.memset" in line:
                line = re_parenthesized_list.sub(_replace_llvm_memset_usage, line,)
        if "declare" in line:
            if "@llvm.memset" in line:
                line = re_parenthesized_list.sub(
                    _replace_llvm_memset_declaration, line,
                )
