...
class Driver(object):
    ...
    def _initialize_extras(self):
        ...
        safe_call = self._wrap_api_call(
            "call_cuIpcOpenMemHandle", call_cuIpcOpenMemHandle
        )
...
class Device(object):
    ...
    def __init__(self, devnum):
        ...
        cc_major = c_int()
        cc_minor = c_int()
        driver.cuDeviceComputeCapability(byref(cc_major), byref(cc_minor), self.id)
    ...
    def COMPUTE_CAPABILITY(self):
        ...
        """
        warnings.warn(
            "Deprecated attribute 'COMPUTE_CAPABILITY'; use lower case version",
            DeprecationWarning,
        )
...
class NumbaCUDAMemoryManager(HostOnlyCUDAMemoryManager):
    ...
    def get_ipc_handle(self, memory):
        ...
        driver.cuIpcGetMemHandle(
            byref(ipchandle), memory.owner.handle,
        )
...
class Context(object):
    ...
    def get_active_blocks_per_multiprocessor(
        ...
        if not flags:
            driver.cuOccupancyMaxActiveBlocksPerMultiprocessor(
                byref(retval), func.handle, blocksize, memsize
            )
        else:
    ...
    def get_max_potential_block_size(
        ...
        if not flags:
            ...
            driver.cuOccupancyMaxPotentialBlockSizeWithFlags(
                byref(gridsize),
                byref(blocksize),
                func.handle,
                b2d_cb,
    ...
    def can_access_peer(self, peer_device):
        ...
        driver.cuDeviceCanAccessPeer(
            byref(can_access_peer), self.device.id, peer_device,
        )
...
class Function(object):
    ...
    def __call__(self, *args):
        ...
        *args -- Must be either ctype objects of DevicePointer instances.
        ...
        launch_kernel(
            self.handle, self.griddim, self.blockdim, self.sharedmem, streamhandle, args
        )
    ...
    def _read_func_attr_all(self):
        ...
        maxtpb = self._read_func_attr(enums.CU_FUNC_ATTRIBUTE_MAX_THREADS_PER_BLOCK)
        return FuncAttr(
...
def launch_kernel(cufunc_handle, griddim, blockdim, sharedmem, hstream, args):
    ...
    driver.cuLaunchKernel(
        cufunc_handle, gx, gy, gz, bx, by, bz, sharedmem, hstream, params, None
    )
...
def _device_pointer_attr(devmem, attr, odata):
    ...
    error = driver.cuPointerGetAttribute(
        byref(odata), attr, device_ctypes_pointer(devmem)
    )
...
def device_pointer_type(devmem):
    ...
    ptrtype = c_int(0)
    _device_pointer_attr(devmem, enums.CU_POINTER_ATTRIBUTE_MEMORY_TYPE, ptrtype)
    map = {
