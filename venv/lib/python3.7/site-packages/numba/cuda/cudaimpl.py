...
def ptx_cmem_arylike(context, builder, sig, args):
    ...
    constvals = [
        context.get_constant(types.byte, i) for i in iter(arr.tobytes(order="A"))
    ]
    constary = lc.Constant.array(Type.int(8), constvals)

    addrspace = nvvm.ADDRSPACE_CONSTANT
    gv = lmod.add_global_variable(
        constary.type, name="_cudapy_cmem", addrspace=addrspace
    )
...
def ptx_match_all_sync(context, builder, sig, args):
    ...
    lmod = builder.module
    fnty = Type.function(
        Type.struct((Type.int(32), Type.int(1))), (Type.int(32), Type.int(width))
    )
    func = lmod.get_or_insert_function(fnty, name=fname)
    return builder.call(func, (mask, value))
...
def ptx_brev_u4(context, builder, sig, args):
    ...
    fn = builder.module.get_or_insert_function(
        lc.Type.function(lc.Type.int(32), (lc.Type.int(32),)), "__nv_brev"
    )
...
def ptx_brev_u8(context, builder, sig, args):
    ...
    fn = builder.module.get_or_insert_function(
        lc.Type.function(lc.Type.int(64), (lc.Type.int(64),)), "__nv_brevll"
    )
    return builder.call(fn, args)
...
def ptx_ffs(context, builder, sig, args):
    return builder.cttz(args[0], context.get_constant(types.boolean, 0))
...
def ptx_max_f4(context, builder, sig, args):
    fn = builder.module.get_or_insert_function(
        lc.Type.function(lc.Type.float(), (lc.Type.float(), lc.Type.float())),
        "__nv_fmaxf",
    )
    return builder.call(fn, args)
...
def ptx_max_f8(context, builder, sig, args):
    fn = builder.module.get_or_insert_function(
        lc.Type.function(lc.Type.double(), (lc.Type.double(), lc.Type.double())),
        "__nv_fmax",
    )
...
def ptx_round(context, builder, sig, args):
    fn = builder.module.get_or_insert_function(
        lc.Type.function(lc.Type.int(64), (lc.Type.double(),)), "__nv_llrint"
    )
    return builder.call(
        fn, [context.cast(builder, args[0], sig.args[0], types.double),]
    )
...
def _generic_array(
    ...
    if dynamic_smem:
        ...
        dynsmem_size = builder.zext(
            builder.call(get_dynshared_size, []), Type.int(width=64)
        )
