...
def compile_kernel(
    ...
    lib, kernel = cres.target_context.prepare_cuda_kernel(
        cres.library, fname, cres.signature.args, debug=debug
    )
...
class DeviceFunctionTemplate(object):
    ...
    def compile(self, args):
        ...
        if args not in self._compileinfos:
            cres = compile_cuda(
                self.py_func, None, args, debug=self.debug, inline=self.inline
            )
...
class DeviceFunction(object):
    def __init__(self, pyfunc, return_type, args, inline, debug):
        ...
        cres.typing_context.insert_user_function(self, device_function_template)
        cres.target_context.insert_user_function(self, cres.fndesc, [cres.library])
    ...
    def _rebuild(cls, func_reduced, return_type, args, inline, debug):
        return cls(
            serialize._rebuild_function(*func_reduced), return_type, args, inline, debug
        )
...
class ForAll(object):
    ...
    def __call__(self, *args):
        ...
        return kernel.configure(
            blkct, tpb, stream=self.stream, sharedmem=self.sharedmem
        )(*args)
...
class CachedPTX(object):
    ...
    def get(self):
        ...
        if ptx is None:
            arch = nvvm.get_arch_option(*cc)
            ptx = nvvm.llvm_to_ptx(self.llvmir, opt=3, arch=arch, **self._extra_options)
            self.cache[cc] = ptx
...
class CachedCUFunction(object):
    ...
    def __reduce__(self):
        ...
        if self.linking:
            msg = (
                "cannot pickle CUDA kernel function with additional "
...
class CUDAKernel(CUDAKernelBase):
    ...
    def __init__(
        ...
        options = {"debug": debug}
        if fastmath:
            options.update(dict(ftz=True, prec_sqrt=False, prec_div=False, fma=True))
    ...
    @classmethod
    def _rebuild(
        cls, name, argtypes, cufunc, link, debug, call_helper, extensions, config
    ):
        """
        Rebuild an instance.
        """
    ...
    def _kernel_call(self, args, griddim, blockdim, stream=0, sharedmem=0):
        ...
        # Configure kernel
        cu_func = cufunc.configure(
            griddim, blockdim, stream=stream, sharedmem=sharedmem
        )
        # Invoke kernel
        cu_func(*kernelargs)
...
class AutoJitCUDAKernel(CUDAKernelBase):
    ...
    def __init__(self, func, bind, targetoptions):
        ...
        # defensive copy
        self.targetoptions["extensions"] = list(
            self.targetoptions.get("extensions", [])
        )
    ...
    def compile(self, sig):
        ...
        if kernel is None:
            ...
            kernel = compile_kernel(self.py_func, argtypes, **self.targetoptions)
    ...
    def inspect_llvm(self, signature=None, compute_capability=None):
        ...
        if signature is not None:
            ...
            return dict(
                (sig, defn.inspect_llvm()) for sig, defn in self.definitions.items()
            )
    ...
    def inspect_asm(self, signature=None, compute_capability=None):
        ...
        if signature is not None:
            ...
            return dict(
                (sig, defn.inspect_asm()) for sig, defn in self.definitions.items()
            )
    ...
    def __reduce__(self):
        ...
        func_reduced = serialize._reduce_function(self.py_func, glbls)
        config = self._serialize_config()
        args = (self.__class__, func_reduced, self.bind, self.targetoptions, config)
        return (serialize._rebuild_reduction, args)
