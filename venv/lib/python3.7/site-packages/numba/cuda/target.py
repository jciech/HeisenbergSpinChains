...
class CUDATargetContext(BaseContext):
    ...
    def prepare_cuda_kernel(self, codelib, fname, argtypes, debug):
        ...
        wrapper = self.generate_kernel_wrapper(library, fname, argtypes, debug=debug)
    ...
    def generate_kernel_wrapper(self, library, fname, argtypes, debug):
        ...
        fnty = Type.function(
            Type.int(), [self.call_conv.get_return_type(types.pyobject)] + argtys
        )
        ...
        def define_error_gv(postfix):
            gv = wrapper_module.add_global_variable(
                Type.int(), name=wrapfn.name + postfix
            )
        ...
        if debug:
            ...
            with builder.if_then(builder.not_(status.is_python_exc)):
                ...
                casfnty = lc.Type.function(old.type, [gv_exc.type, old.type, old.type])

                casfn = wrapper_module.add_function(casfnty, name="___numba_cas_hack")
    ...
    def insert_const_string(self, mod, string):
        ...
        if gv is None:
            ...
            gv = mod.add_global_variable(
                text.type, name=name, addrspace=nvvm.ADDRSPACE_CONSTANT
            )
    ...
    def insert_string_const_addrspace(self, builder, string):
        ...
        return self.insert_addrspace_conv(builder, gv, nvvm.ADDRSPACE_CONSTANT)
