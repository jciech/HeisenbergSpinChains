...
def declare_atomic_add_float32(lmod):
    ...
    fnty = lc.Type.function(
        lc.Type.float(), (lc.Type.pointer(lc.Type.float(), 0), lc.Type.float())
    )
...
def declare_atomic_add_float64(lmod):
    ...
    fnty = lc.Type.function(
        lc.Type.double(), (lc.Type.pointer(lc.Type.double()), lc.Type.double())
    )
    return lmod.get_or_insert_function(fnty, fname)
...
def declare_atomic_max_float32(lmod):
    ...
    fnty = lc.Type.function(
        lc.Type.float(), (lc.Type.pointer(lc.Type.float()), lc.Type.float())
    )
    return lmod.get_or_insert_function(fnty, fname)
...
def insert_addrspace_conv(lmod, elemtype, addrspace):
    ...
    s2g_fnty = lc.Type.function(elem_ptr_ty, [elem_ptr_ty_addrspace])
...
def declare_string(builder, value):
    ...
    gl = lmod.add_global_variable(
        cval.type, name="_str", addrspace=nvvm.ADDRSPACE_CONSTANT
    )
