...
class TestGetItem(TestCase):
    ...
    def test_2d_integer_indexing(
        ...
        arraytype = types.Array(types.int32, 2, "C")
        cr = compile_isolated(
            pyfunc, (arraytype, types.int32, types.int32), flags=flags
        )
        cfunc = cr.entry_point
    ...
    def test_2d_float_indexing(self, flags=enable_pyobj_flags):
        ...
        arraytype = types.Array(types.int32, 2, "C")
        cr = compile_isolated(
            pyfunc, (arraytype, types.float32, types.int32), flags=flags
        )
        cfunc = cr.entry_point
    ...
    def check_ellipsis(self, pyfunc, flags):
        def compile_func(arr):
            cr = compile_isolated(
                pyfunc, (typeof(arr), types.intp, types.intp), flags=flags
            )
...
class TestSetItem(TestCase):
    ...
    def test_setitem_readonly(self):
        ...
        with self.assertRaises((TypeError, errors.TypingError)) as raises:
            setitem_usecase(arr, 1, 42)
        self.assertIn(
            "Cannot modify value of type readonly array", str(raises.exception)
        )
