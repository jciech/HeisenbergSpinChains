...
class TestTypingError(unittest.TestCase):
    ...
    def test_issue_868(self):
        ...
        expected = (
            "Invalid use of Function(<built-in function mul>) with argument(s) of"
            " type(s): (UniTuple({0} x 1), {1})".format(
                str(types.intp), types.IntegerLiteral(2)
            )
        )
    ...
    def test_bad_hypot_usage(self):
        ...
        self.assertTrue(
            re.search(
                r"\[1\] During: resolving callee type: Function.*hypot", ctx_lines[0]
            )
        )
        self.assertTrue(
            re.search(
                r"\[2\] During: typing of call .*test_typingerror.py", ctx_lines[1]
            )
        )
    ...
    def test_imprecise_list(self):
        ...
        errmsg = str(raises.exception)
        msg = (
            "Cannot infer the type of variable 'l', have imprecise type: "
            "list(undefined)"
        )
    ...
    def test_array_setitem_invalid_cast(self):
        ...
        errmsg = str(raises.exception)
        self.assertIn(
            "Invalid use of Function({})".format(operator.setitem), errmsg,
        )
        self.assertIn(
            "(array(float64, 1d, C), Literal[int](0), complex128)", errmsg,
        )
    ...
    def test_template_rejection_error_message_cascade(self):
        ...
        ctx_lines = [x for x in errmsg.splitlines() if "] During" in x]
        search = [
            r"\[1\] During: resolving callee type: Function.*enumerate",
