...
class TestUnicode(BaseTest):
    ...
    def _check_ordering_op(self, usecase):
        ...
        for a in UNICODE_ORDERING_EXAMPLES:
            self.assertEqual(
                pyfunc(a, a), cfunc(a, a), '%s: "%s", "%s"' % (usecase.__name__, a, a),
            )

        # Check comparison to adjacent
        for a, b in permutations(UNICODE_ORDERING_EXAMPLES, r=2):
            self.assertEqual(
                pyfunc(a, b), cfunc(a, b), '%s: "%s", "%s"' % (usecase.__name__, a, b),
            )
            # and reversed
            self.assertEqual(
                pyfunc(b, a), cfunc(b, a), '%s: "%s", "%s"' % (usecase.__name__, b, a),
            )
    ...
    def test_repeat_exception_float(self):
        ...
        with self.assertRaises(TypingError) as raises:
            cfunc("hi", 2.5)
        self.assertIn(
            "Invalid use of Function(<built-in function mul>)", str(raises.exception)
        )
    ...
    def test_join_non_string_exception(self):
        ...
        self.assertIn(
            "During: resolving callee type: BoundFunction", exc_message,
        )
    ...
    @unittest.skipUnless(_py37_or_later, "isascii method requires Python 3.7 or later")
