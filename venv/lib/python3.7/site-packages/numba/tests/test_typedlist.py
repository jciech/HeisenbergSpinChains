...
class TestTypedList(MemoryLeakMixin, TestCase):
    ...
    def test_dtype_raises_exception_on_untyped_list(self):
        ...
        with self.assertRaises(RuntimeError) as raises:
            ...
            L._dtype
        self.assertIn(
            "invalid operation on untyped list", str(raises.exception),
        )
    ...
    def test_setitem_slice_value_error(self):
        ...
        with self.assertRaises(ValueError) as raises:
            tl[8:3:-1] = assignment
        self.assertIn(
            "length mismatch for extended slice and sequence", str(raises.exception),
        )
    ...
    def test_catch_global_typed_list(self):
        ...
        with self.assertRaises(TypingError) as raises:
            foo()
        self.assertIn(
            expected_message, str(raises.exception),
        )
    ...
    def test_none_typed_method_fails(self):
        ...
        for line1, line2 in (
            ...
            with self.assertRaises(TypingError) as raises:
                ...
                foo()
            self.assertIn(
                "method support for List[None] is limited", str(raises.exception),
            )
...
class TestListSort(MemoryLeakMixin, TestCase):
    ...
    def test_sort_dispatcher_key(self):
        ...
        self.assertEqual(
            list(udt(my_lists["nb"], key=nb_key)), udt(my_lists["py"], key=py_key),
        )
...
class TestListFromIter(MemoryLeakMixin, TestCase):
    ...
    def test_exception_on_plain_int(self):
        ...
        with self.assertRaises(TypingError) as raises:
            foo()
        self.assertIn(
            "List() argument must be iterable", str(raises.exception),
        )
    ...
    def test_exception_on_too_many_args(self):
        ...
        with self.assertRaises(TypingError) as raises:
            foo()
        self.assertIn(
            "List() expected at most 1 argument, got 2", str(raises.exception),
        )
        ...
        with self.assertRaises(TypingError) as raises:
            foo()
        self.assertIn(
            "List() expected at most 1 argument, got 3", str(raises.exception),
        )
    ...
    def test_exception_on_kwargs(self):
        ...
        with self.assertRaises(TypingError) as raises:
            foo()
        self.assertIn(
            "List() takes no keyword arguments", str(raises.exception),
        )
