...
class TestGenerators(MemoryLeakMixin, TestCase):
    ...
    def test_gen5(self):
        ...
        self.assertIn(
            "Cannot type generator: it does not yield any value", str(cm.exception)
        )
    ...
    def check_gen7(self, flags=no_pyobj_flags):
        pyfunc = gen7
        cr = compile_isolated(
            pyfunc, (types.Array(types.float64, 1, "C"),), flags=flags
        )
    ...
    def check_ndindex(self, flags=no_pyobj_flags):
        pyfunc = gen_ndindex
        cr = compile_isolated(pyfunc, (types.UniTuple(types.intp, 2),), flags=flags)
        shape = (2, 3)
        pygen = pyfunc(shape)
    ...
    def check_np_flat(self, pyfunc, flags=no_pyobj_flags):
        cr = compile_isolated(pyfunc, (types.Array(types.int32, 2, "C"),), flags=flags)
        arr = np.arange(6, dtype=np.int32).reshape((2, 3))
        self.check_generator(pyfunc(arr), cr.entry_point(arr))
        cr = compile_isolated(pyfunc, (types.Array(types.int32, 2, "A"),), flags=flags)
        arr = arr.T
        self.check_generator(pyfunc(arr), cr.entry_point(arr))
...
class TestNrtArrayGen(MemoryLeakMixin, TestCase):
    def test_nrt_gen0(self):
        ...
        self.assertEqual(sys.getrefcount(py_ary), sys.getrefcount(c_ary))
    ...
    def test_nrt_gen1(self):
        ...
        self.assertEqual(py_res, c_res)
        # Check reference count
        self.assertEqual(sys.getrefcount(py_ary1), sys.getrefcount(c_ary1))
        self.assertEqual(sys.getrefcount(py_ary2), sys.getrefcount(c_ary2))
...
class TestNrtNestedGen(TestCase):
    def test_nrt_nested_gen(self):
        ...
        self.assertEqual(sys.getrefcount(py_res), sys.getrefcount(c_res))
    ...
    def test_nrt_nested_gen_refct(self):
        ...
        self.assertEqual(sys.getrefcount(py_old), sys.getrefcount(c_old))
