...
class BaseComplexTest(object):
    ...
    def run_unary(
        self, pyfunc, x_types, x_values, ulps=1, abs_tol=None, flags=enable_pyobj_flags
    ):
        for tx in x_types:
            ...
            for vx in x_values:
                ...
                self.assertPreciseEqual(
                    got, expected, prec=prec, ulps=ulps, abs_tol=abs_tol, msg=msg
                )
...
class TestComplex(BaseComplexTest, TestCase):
    def test_real(self, flags=enable_pyobj_flags):
        ...
        self.run_unary(
            real_usecase, [types.float32, types.float64], [1.5, -0.5], flags=flags
        )
    ...
    def test_imag(self, flags=enable_pyobj_flags):
        ...
        self.run_unary(
            imag_usecase, [types.float32, types.float64], [1.5, -0.5], flags=flags
        )
    ...
    def test_conjugate(self, flags=enable_pyobj_flags):
        ...
        self.run_unary(
            conjugate_usecase, [types.int8, types.int64], [1, 0, -3], flags=flags
        )
        self.run_unary(
            conjugate_usecase, [types.float32, types.float64], [1.5, -0.5], flags=flags
        )
...
class TestCMath(BaseComplexTest, TestCase):
    ...
    def test_atan(self):
        self.check_unary_func(
            atan_usecase, enable_pyobj_flags, ulps=2,
        )

    def test_atan_npm(self):
        self.check_unary_func(
            atan_usecase, no_pyobj_flags, ulps=2,
        )
