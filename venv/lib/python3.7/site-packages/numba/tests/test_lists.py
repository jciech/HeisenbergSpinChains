...
class TestLists(MemoryLeakMixin, TestCase):
    ...
    @unittest.skipUnless(
        sys.maxsize >= 2 ** 32, "need a 64-bit system to test for MemoryError"
    )
    def test_mul_error(self):
        ...
        if sys.platform.startswith("darwin"):
            ...
            libc.printf(
                "###Please ignore the above error message i.e. can't allocate region."
                " It is in fact the purpose of this test to request more memory than"
                " can be provided###\n".encode("UTF-8")
            )
...
class TestListOfList(ManagedListTestCase):
    ...
    def test_heterogeneous_list_error(self):
        ...
        self.assertIn(
            "reflected list(array(float64, 1d, C)) != reflected list((int64 x 1))",
            str(raises.exception),
        )
        ...
        with self.assertRaises(ValueError) as raises:
            cfunc(l5)
        self.assertIn(
            "Cannot type list element of <class 'dict'>", str(raises.exception)
        )
    ...
    def test_c02(self):
        ...
        with self.assertRaises(errors.TypingError) as raises:
            self.compile_and_test(bar, r)
        self.assertIn(
            "Invalid use of BoundFunction(list.append", str(raises.exception),
        )
