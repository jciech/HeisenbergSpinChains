...
class TestUFuncs(BaseUFuncTest, TestCase):
    def unary_ufunc_test(
        ...
        for input_tuple in inputs:
            ...
            cr = self.cache.compile(pyfunc, (input_type, output_type), flags=flags)
            cfunc = cr.entry_point

            if isinstance(input_operand, np.ndarray):
                result = np.zeros(input_operand.size, dtype=output_type.dtype.name)
                expected = np.zeros(input_operand.size, dtype=output_type.dtype.name)
            else:
            ...
            try:
                np.testing.assert_array_almost_equal(
                    expected, result, decimal=5, err_msg=msg
                )
    ...
    def binary_ufunc_test(
        ...
        for input_tuple in inputs:
            ...
            cr = self.cache.compile(
                pyfunc, (input_type, input_type, output_type), flags=flags
            )
            cfunc = cr.entry_point

            if isinstance(lhs, np.ndarray):
                result = np.zeros(lhs.size, dtype=output_type.dtype.name)
                expected = np.zeros(lhs.size, dtype=output_type.dtype.name)
            else:
                result = np.zeros(1, dtype=output_type.dtype.name)
                expected = np.zeros(1, dtype=output_type.dtype.name)
    ...
    def test_true_divide_ufunc(self, flags=no_pyobj_flags):
        self.binary_ufunc_test(
            np.true_divide, flags=flags, int_output_type=types.float64
        )
    ...
    def test_arctanh_ufunc(self, flags=no_pyobj_flags):
        ...
        self.unary_ufunc_test(np.arctanh, skip_inputs=to_skip, flags=flags, kinds="cf")
    ...
    def binary_ufunc_mixed_types_test(self, ufunc, flags=no_pyobj_flags):
        ...
        for input1, input2, output_type in itertools.product(
            inputs1, inputs2, output_types
        ):
            ...
            cr = self.cache.compile(
                pyfunc, (input1_type, input2_type, output_type), flags=flags
            )
            cfunc = cr.entry_point
    ...
    def test_broadcasting(self):
        ...
        for x, result in zip(input_operands, output_operands):
            ...
            cr = self.cache.compile(
                pyfunc, (input_type, output_type), flags=no_pyobj_flags
            )
            cfunc = cr.entry_point
        ...
        for x, y in itertools.product(input1_operands, input2_operands):
            ...
            cr = self.cache.compile(
                pyfunc, (input1_type, input2_type, output_type), flags=no_pyobj_flags
            )
            cfunc = cr.entry_point
    ...
    def test_implicit_output_npm(self):
        with self.assertRaises(TypeError):
            ...
            cr = compile_isolated(myadd, (arr_ty, arr_ty), flags=no_pyobj_flags)
    ...
    def test_broadcast_implicit_output_npm_nrt(self):
        ...
        for x, y in itertools.product(input1_operands, input2_operands):
            ...
            cr = self.cache.compile(
                pyfunc, (input1_type, input2_type), flags=enable_nrt_flags
            )
            cfunc = cr.entry_point
    ...
    def test_implicit_output_layout_binary(self):
        ...
        for arg0, arg1 in testcases:
            cr = self.cache.compile(
                pyfunc, (typeof(arg0), typeof(arg1)), flags=enable_nrt_flags
            )
    ...
    def test_implicit_output_layout_unary(self):
        ...
        for arg0 in [X, Y, Z]:
            cr = self.cache.compile(pyfunc, (typeof(arg0),), flags=enable_nrt_flags)
...
class TestArrayOperators(BaseUFuncTest, TestCase):
    ...
    def unary_op_test(
        ...
        for input_tuple in inputs:
            ...
            cr = self.cache.compile(pyfunc, (input_type,), flags=flags)
            cfunc = cr.entry_point
    ...
    def binary_op_test(
        ...
        for input_tuple in inputs:
            ...
            cr = self.cache.compile(pyfunc, (input_type0, input_type1), flags=flags)
            cfunc = cr.entry_point
    ...
    def inplace_op_test(self, operator, lhs_values, rhs_values, lhs_dtypes, rhs_dtypes):
        ...
        for lhs, rhs in itertools.product(lhs_inputs, rhs_inputs):
            lhs_type = typeof(lhs)
            rhs_type = typeof(rhs)
            cr = self.cache.compile(pyfunc, (lhs_type, rhs_type), flags=no_pyobj_flags)
            cfunc = cr.entry_point
            expected = lhs.copy()
            pyfunc(expected, rhs)
            got = lhs.copy()
