...
class pyStencilGenerator:
    ...
    class Builder:
            ...
            new = ast.Assign(
                targets=[
                    ast.Subscript(
                        ...
                        slice=ast.Index(
                            value=ast.Tuple(elts=elts_info, ctx=ast.Load())
                        ),
                        ctx=ast.Store(),
                    )
                ],
        ...
        def gen_attr(self, name, attr):
            ...
            return ast.Attribute(
                value=ast.Name(id=name, ctx=ast.Load()), attr=attr, ctx=ast.Load()
            )
        ...
        def gen_call(self, call_name, args, kwargs=None):
            ...
            if kwargs is not None:
                keywords = [
                    ast.keyword(arg="%s" % x, value=ast.parse(str(x)).body[0].value)
                    for x in kwargs
                ]
            else:
                keywords = []
    ...
    class FixRelIndex(ast.NodeTransformer, Builder):
        ...
        def __init__(
            ...
            **kwargs
            ...
            self._standard_indexing = standard_indexing if standard_indexing else []
        ...
        def get_val_from_num(self, node):
            ...
            if isinstance(node, ast.Num):
                ...
                raise ValueError("get_val_from_num: Unknown indexing operation")
        ...
        def visit_Subscript(self, node):
            ...
            def handle2dindex(node):
                ...
                for x, val in enumerate(node.slice.value.elts):
                    ...
                    idx.append(
                        ast.BinOp(
                            left=ast.Name(
                                id=self._id_pat % self.ids[x], ctx=ast.Load()
                            ),
                            op=ast.Add(),
                            right=useval,
                            ctx=ast.Load(),
                ...
                if self._idx_len == -1:
                    ...
                    if self._idx_len != len(idx):
                        raise ValueError("Relative indexing mismatch detected")
                if isinstance(node.ctx, ast.Store):
                    msg = (
                        "Assignments to array passed to stencil kernels is not allowed"
                    )
                ...
                newnode = ast.Subscript(
                    value=node.value,
                    slice=ast.Index(
                        value=ast.Tuple(elts=idx, ctx=ast.Load()), ctx=ast.Load()
                    ),
                    ctx=context,
                )
                ...
                if not self._neighborhood:
                    for x, lnode in enumerate(node.slice.value.elts):
                        if isinstance(lnode, ast.Num) or isinstance(lnode, ast.UnaryOp):
                            relvalue = self.get_val_from_num(lnode)
                        elif hasattr(lnode, "id") and lnode.id in self._const_assigns:
            ...
            def handle1dindex(node):
                useval = self._const_assigns.get(node.slice.value, node.slice.value)
                idx = ast.BinOp(
                    left=ast.Name(id=self._id_pat % self.ids[0], ctx=ast.Load()),
                    op=ast.Add(),
                    right=useval,
                    ctx=ast.Load(),
                )
                ...
                newnode = ast.Subscript(
                    value=node.value,
                    slice=ast.Index(value=idx, ctx=ast.Load()),
                    ctx=context,
                )
                ast.copy_location(newnode, node)
                ast.fix_missing_locations(newnode)
                ...
                if not self._neighborhood:
                    if isinstance(node.slice.value, ast.Num) or isinstance(
                        node.slice.value, ast.UnaryOp
                    ):
                        relvalue = self.get_val_from_num(node.slice.value)
                    elif (
                        hasattr(node.slice.value, "id")
                        and node.slice.value.id in self._const_assigns
                    ):
            ...
            def computeSlice(i, node):
                def gen_idx(val, x):
                    ...
                    tmp = ast.BinOp(
                        left=ast.Name(id=self._id_pat % self.ids[x], ctx=ast.Load()),
                        op=ast.Add(),
                        right=useval,
                        ctx=ast.Load(),
                    )
                ...
                newnode = ast.Slice(
                    gen_idx(node.lower, i), gen_idx(node.upper, i), node.step
                )
                ast.copy_location(newnode, node)
                ast.fix_missing_locations(newnode)
                return newnode
            ...
            def computeIndex(i, node):
                ...
                idx = ast.BinOp(
                    left=ast.Name(id=self._id_pat % self.ids[i], ctx=ast.Load()),
                    op=ast.Add(),
                    right=useval,
                    ctx=ast.Load(),
                )
            ...
            def handleExtSlice(node):
                ...
                newnode = ast.Subscript(
                    value=node.value,
                    slice=ast.ExtSlice(dims=idx, ctx=ast.Load()),
                    ctx=context,
                )
                    ...
                    for x, anode in enumerate(node.slice.dims):
                        if isinstance(anode, ast.Slice):
                            for lnode in [anode.lower, anode.upper]:
                                if isinstance(lnode, ast.Num) or isinstance(
                                    lnode, ast.UnaryOp
                                ):
                                    relvalue = self.get_val_from_num(lnode)
                                elif (
                                    hasattr(lnode, "id")
                                    and lnode.id in self._const_assigns
                                ):
                                    relvalue = self._const_assigns[lnode.id]
                                else:
                                    raise ValueError("Cannot interpret indexing value")
                            ...
                            val = anode.value
                            if isinstance(val, ast.Num) or isinstance(val, ast.UnaryOp):
                                relvalue = self.get_val_from_num(val)
                            elif hasattr(val, "id") and val.id in self._const_assigns:
            ...
            def handleSlice(node):
                ...
                if isinstance(node.ctx, ast.Store):
                    ...
                    raise ValueError(msg)
                context = ast.Load()
                newnode = ast.Subscript(value=node.value, slice=idx, ctx=context)
                ast.copy_location(newnode, node)
                ast.fix_missing_locations(newnode)
        ...
        def idx_len(self):
            if self._idx_len == -1:
                raise ValueError("Transform has not been run/no indexes found")
            else:
    ...
    class FixFunc(ast.NodeTransformer, Builder):
        ...
        * Function definition as an entry point
        ...
        def visit_FunctionDef(self, node):
            ...
            check_impl = ast.parse(checksrc.strip()).body[0]  # don't need module
            ast.fix_missing_locations(check_impl)

            checker_call = self.gen_call(
                "check_stencil_arrays", self._relidx_args, kwargs=["neighborhood"]
            )
    ...
    class GetKernelProps(ast.NodeVisitor, Builder):
        ...
        def visit_FunctionDef(self, node):
            ...
            self._retty = ast.Assign(
                targets=[ast.Name(id="__retdtype", ctx=ast.Store())],
                value=compute_retdtype.value,
            )
    ...
    def generate_stencil_tree(self, func, cval, standard_indexing, neighborhood):
        """
        ...
        def pipeline(tree):
            ...
            if standard_indexing:
                for x in standard_indexing:
                    if x not in argnm:
                        msg = "Non-existent variable specified in standard_indexing"
                        raise ValueError(msg)
...
def pyStencil(func_or_mode="constant", **options):
    ...
    tree = gen.generate_stencil_tree(func, cval, standard_indexing, neighborhood)
...
class TestManyStencils(TestStencilBase):
        ...
        * from the njit(parallel=True) of a trivial wrapper function around
        ...
        def errorhandler(exty=None, usecase=None):
            try:
                ...
                if exty is not None:
                    lexty = (
                        exty if hasattr(exty, "__iter__") else [exty,]
                    )
                    found = False
                    for ex in lexty:
        ...
        try:
            ...
            expected = ast_impl(*args, neighborhood=options.get("neighborhood"))
            if DEBUG_OUTPUT:
                print("\nExpected:\n", expected)
        ...
        if not expected_present:
            if expected_exception is None:
                raise RuntimeError(
                    "pyStencil failed, was not caught/expected", pyStencil_unhandled_ex
                )
    ...
    def test_basic35(self):
        ...
        ex = self.exception_dict(
            stencil=ValueError, parfor=ValueError, njit=LoweringError
        )
    ...
    def test_basic36(self):
        ...
        a = np.arange(12.0).reshape(3, 4)
        self.check(kernel, a, options={"cval": 5.0})
    ...
    def test_basic37(self):
        ...
        a = np.arange(12.0).reshape(3, 4)
        self.check(kernel, a, options={"cval": 5 + 63.0})
    ...
    def test_basic38(self):
        ...
        self.check(kernel, a, options={"cval": 1.0j}, expected_exception=ex)
    ...
    def test_basic39(self):
        ...
        a = np.arange(12.0).reshape(3, 4)
        self.check(kernel, a, options={"cval": np.sin(3.0) + np.cos(2)})
    ...
    def test_basic41(self):
        ...
        self.check(kernel, a, b, expected_exception=[ValueError, AssertionError])
    ...
    def test_basic51(self):
        ...
        self.check(
            ...
            expected_exception=[ValueError, LoweringError],
        )
    ...
    def test_basic53(self):
        ...
        b = np.arange(12.0).reshape(3, 4)
        ex = self.exception_dict(
            pyStencil=ValueError, stencil=Exception, parfor=ValueError, njit=Exception
        )
        self.check(
    ...
    def test_basic62(self):
        ...
        a = np.arange(12.0).reshape(3, 4)
        b = np.arange(12.0).reshape(3, 4)
        self.check(kernel, a, b, options={"standard_indexing": "b", "cval": 10.0})
    ...
    def test_basic78(self):
        ...
        self.check(kernel, a, b, options={"neighborhood": ((-6, -3), (-7, -2),)})
    ...
    def test_basic82(self):
        ...
        self.check(
            ...
            a,
            b,
            options={"neighborhood": ((-3, 0), (-3, 0)), "standard_indexing": "b"},
        )
    ...
    def test_basic88(self):
        ...
        self.check(kernel, a, 1.0, options={}, expected_exception=ex)
