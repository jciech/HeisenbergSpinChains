...
class TestGeneratedDispatcher(TestCase):
    ...
    def test_signature_errors(self):
        ...
        with self.assertRaises(TypeError) as raises:
            f(1j)
        self.assertIn(
            "should be compatible with signature '(x, y=5)', but has signature '(x)'",
        ...
        with self.assertRaises(TypeError) as raises:
            f(1)
        self.assertIn(
            "should be compatible with signature '(x, y=5)', "
...
class TestCache(BaseCacheUsecasesTest):
    ...
    def test_looplifted(self):
        ...
        self.assertIn(
            'Cannot cache compiled function "looplifted" as it uses lifted loops',
            str(w[0].message),
        )
    ...
    def test_big_array(self):
        ...
        self.assertEqual(len(w), 1)
        self.assertIn(
            'Cannot cache compiled function "use_big_array" as it uses dynamic globals',
            str(w[0].message),
        )
    ...
    def test_closure(self):
        ...
        for item in w:
            self.assertIn('Cannot cache compiled function "closure"', str(item.message))
