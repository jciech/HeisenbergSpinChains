...
class TestDictObject(MemoryLeakMixin, TestCase):
    ...
    def test_dict_popitem(self):
        ...
        for i in range(1, len(keys)):
            self.assertEqual(
                foo(keys[:i], vals[:i]), (keys[i - 1], vals[i - 1]),
            )
    ...
    def test_dict_popitem_many(self):
        ...
        for i in range(len(keys)):
            self.assertEqual(
                foo(keys, vals, npop=3), core.py_func(dict(zip(keys, vals)), npop=3),
            )
    ...
    def test_dict_pop_many(self):
        ...
        self.assertEqual(
            foo(keys, vals, pops), core.py_func(dict(zip(keys, vals)), pops),
        )
    ...
    def test_dict_items(self):
        ...
        self.assertEqual(
            foo(keys, vals), list(zip(keys, vals)),
        )
    ...
    def test_dict_contains(self):
        ...
        self.assertEqual(
            foo(keys, vals, [2, 3, 4, 1, 0]), [True, True, False, True, False],
        )
    ...
    def test_001_cannot_downcast_key(self):
        ...
        with self.assertRaises(TypingError) as raises:
            foo(10)
        self.assertIn(
            "cannot safely cast complex128 to int32", str(raises.exception),
        )
    ...
    def test_002_cannot_downcast_default(self):
        ...
        with self.assertRaises(TypingError) as raises:
            foo(10)
        self.assertIn(
            "cannot safely cast complex128 to float64", str(raises.exception),
        )
    ...
    def test_003_cannot_downcast_key(self):
        ...
        with self.assertRaises(TypingError) as raises:
            foo(10)
        self.assertIn(
            "cannot safely cast float64 to int32", str(raises.exception),
        )
...
class TestDictRefctTypes(MemoryLeakMixin, TestCase):
    ...
    def test_dict_of_dict_int_keyval(self):
        ...
        d = Dict.empty(
            key_type=types.intp, value_type=types.DictType(types.intp, types.intp),
        )
...
class TestDictInferred(TestCase):
    ...
    def test_conflicting_value_type(self):
        ...
        with self.assertRaises(TypingError) as raises:
            foo(k, v, w)
        self.assertIn(
            "cannot safely cast float64 to {}".format(typeof(v)), str(raises.exception),
        )
    ...
    def test_dict_of_dict(self):
        ...
        d = foo(k1, k2, v)
        self.assertEqual(
            dict(d), {k1: {k1 + 1: k1 + v}, k2: {k2 + 2: k2 + v},},
        )
