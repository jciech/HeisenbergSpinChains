...
from numba.tests.support import TestCase, is_parfors_unsupported, skip_ppc64le_issue4026
...
class BaseTest(TestCase):
    ...
    def run_parfor_pre_pass(cls, test_func, args):
        tp, options, diagnostics, preparfor_pass = cls._run_parfor(test_func, args)
...
class TestConvertLoopPass(BaseTest):
    ...
    def test_prange_map_overwrite_index(self):
        ...
        with self.assertRaises(errors.UnsupportedRewriteError) as raises:
            self.run_parfor_sub_pass(test_impl, ())
        self.assertIn(
            "Overwrite of parallel loop index", str(raises.exception),
        )
