...
class TestBuiltins(TestCase):
    ...
    def test_filter(self, flags=enable_pyobj_flags):
        ...
        cr = compile_isolated(
            pyfunc, (types.Dummy("list"), types.Dummy("function_ptr")), flags=flags
        )
        cfunc = cr.entry_point
    ...
    def test_format(self, flags=enable_pyobj_flags):
        ...
        cr = compile_isolated(pyfunc, (types.string, types.float32,), flags=flags)
        cfunc = cr.entry_point
        x = "{0}"
        for y in [-1.1, 0.0, 1.1]:
            self.assertPreciseEqual(cfunc(x, y), pyfunc(x, y))

        cr = compile_isolated(pyfunc, (types.string, types.string,), flags=flags)
        cfunc = cr.entry_point
        x = "{0}"
        for y in ["a", "b", "c"]:
            self.assertPreciseEqual(cfunc(x, y), pyfunc(x, y))
    ...
    def test_reduce(self, flags=enable_pyobj_flags):
        ...
        cr = compile_isolated(
            pyfunc, (types.Dummy("function_ptr"), types.Dummy("list")), flags=flags
        )
        cfunc = cr.entry_point
    ...
    def test_pow_op_usecase(self):
        ...
        for x, y in args:
            cres = compile_isolated(
                pow_op_usecase, (typeof(x), typeof(y)), flags=no_pyobj_flags
            )
    ...
    def test_pow_usecase(self):
        ...
        for x, y in args:
            cres = compile_isolated(
                pow_usecase, (typeof(x), typeof(y)), flags=no_pyobj_flags
            )
            r = cres.entry_point(x, y)
