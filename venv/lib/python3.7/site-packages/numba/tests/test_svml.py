...
class TestSVMLGeneration(TestCase):
    ...
    def autogenerate(cls):
        ...
        test_flags = sum(
            [list(combinations(test_flags, x)) for x in range(len(test_flags) + 1)], []
        )
        ...
        for ft in test_flags:
            ...
            for f in ft:
                flags.set(f, {"fastmath": cpu.FastMathOptions(True)}.get(f, True))
...
class TestSVML(TestCase):
    ...
    def check(self, pyfunc, *args, **kwargs):
        ...
        # look for specific patters in the asm for a given target
        with override_env_config("NUMBA_CPU_NAME", cpu_name), override_env_config(
            "NUMBA_CPU_FEATURES", ""
        ):
            # recompile for overridden CPU
            jitstd, jitfast = self.compile(pyfunc, *args)
