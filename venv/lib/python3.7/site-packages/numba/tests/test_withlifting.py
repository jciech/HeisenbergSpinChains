...
from numba.tests.support import MemoryLeak, TestCase, captured_stdout, skip_unless_scipy
import unittest
...
class TestLiftObj(MemoryLeak, TestCase):
    ...
    def test_case04_bogus_variable_type_info(self):
        ...
        with self.assertRaises(errors.TypingError) as raises:
            cfoo(x)
        self.assertIn(
            "Invalid type annotation on non-outgoing variables", str(raises.exception),
        )
    ...
    def test_case07_mystery_key_error(self):
        ...
        with self.assertRaises(errors.TypingError) as raises:
            cfoo(x)
        self.assertIn(
            "missing type annotation on outgoing variables", str(raises.exception),
        )
    ...
    def test_case11_define_function_in_context(self):
        ...
        with self.assertRaises(NameError) as raises:
            cfoo(x)
        self.assertIn(
            "global name 'bar' is not defined", str(raises.exception),
        )
    ...
    def test_case13_branch_to_objmode_ctx(self):
        ...
        for each in w:
            self.assertFalse(
                each.filename.endswith("dataflow.py"),
...
class TestBogusContext(BaseTestWithLifting):
    def test_undefined_global(self):
        ...
        self.assertIn(
            "Undefined variable used as context manager", str(raises.exception),
        )
    ...
    def test_invalid(self):
        ...
        self.assertIn(
            "Unsupported context manager in use", str(raises.exception),
        )
