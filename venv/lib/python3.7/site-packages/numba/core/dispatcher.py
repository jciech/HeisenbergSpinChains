        ...
        try:
            ...
            if failed_args:
                ...
                msg = str(e).rstrip() + (
                    ...
                    " argument(s):\n%s\n"
                    % "\n".join(
                        "- argument %d: %s" % (i, err) for i, err in failed_args
                    )
                )
                e.patch_message(msg)
...
class Dispatcher(_DispatcherBase):
    ...
    def dump(self, tab=""):
        print(
            f"{tab}DUMP {type(self).__name__}[{self.py_func.__name__}, type"
            f" code={self._type._code}]"
        )
        for cres in self.overloads.values():
            cres.dump(tab=tab + "  ")
    ...
    @classmethod
    def _rebuild(
        cls, uuid, func_reduced, locals, targetoptions, impl_kind, can_compile, sigs
    ):
        """
