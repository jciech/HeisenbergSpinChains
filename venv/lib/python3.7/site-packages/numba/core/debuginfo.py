...
class DIBuilder(AbstractDIBuilder):
    ...
    def mark_subprogram(self, function, name, loc):
        di_subp = self._add_subprogram(
            name=name, linkagename=function.name, line=loc.line
        )
    ...
    def _di_subroutine_type(self):
        return self.module.add_debug_info(
            "DISubroutineType", {"types": self.module.add_metadata([]),}
        )
    ...
    def _di_location(self, line):
        return self.module.add_debug_info(
            "DILocation", {"line": line, "column": 1, "scope": self.subprograms[-1],}
        )
...
class NvvmDIBuilder(DIBuilder):
    ...
    def mark_location(self, builder, loc):
        ...
        asm = ir.InlineAsm(asmty, "// dbg {}".format(loc.line), "", side_effect=True)
        call = builder.call(asm, [])
        md = self._di_location(loc.line)
        call.set_metadata("numba.dbg", md)

    def mark_subprogram(self, function, name, loc):
        self._add_subprogram(name=name, linkagename=function.name, line=loc.line)
    ...
    def _filepair(self):
        return self.module.add_metadata(
            [os.path.basename(self.filepath), os.path.dirname(self.filepath),]
        )

    def _di_file(self):
        return self.module.add_metadata(
            [self._const_int(self.DI_File), self._filepair(),]
        )
