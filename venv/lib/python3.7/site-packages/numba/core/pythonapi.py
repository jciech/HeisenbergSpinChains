...
class PythonAPI(object):
    ...
    def emit_environment_sentry(self, envptr, return_pyobject=False):
        ...
        with cgutils.if_unlikely(self.builder, is_null):
            if return_pyobject:
                ...
                self.err_set_string("PyExc_RuntimeError", "missing Environment")
    ...
    def dict_new(self, presize=0):
        if presize == 0:
            ...
            return self.builder.call(fn, [Constant.int(self.py_ssize_t, presize)])
    ...
    def slice_as_ints(self, obj):
        ...
        fnty = Type.function(
            Type.int(), [self.pyobj] + [self.py_ssize_t.as_pointer()] * 3
        )
    ...
    def sequence_getslice(self, obj, start, stop):
        fnty = Type.function(self.pyobj, [self.pyobj, self.py_ssize_t, self.py_ssize_t])
    ...
    def list_setslice(self, lst, start, stop, obj):
        ...
        fnty = Type.function(
            Type.int(), [self.pyobj, self.py_ssize_t, self.py_ssize_t, self.pyobj]
        )
    ...
    def set_iterate(self, set):
        ...
        posptr = cgutils.alloca_once_value(
            builder, ir.Constant(self.py_ssize_t, 0), name="posptr"
        )
    ...
    def nrt_adapt_ndarray_to_python(self, aryty, ary, dtypeptr):
        ...
        intty = ir.IntType(32)
        fnty = Type.function(self.pyobj, [self.voidptr, intty, intty, self.pyobj])
        ...
        return self.builder.call(
            fn, [self.builder.bitcast(aryptr, self.voidptr), ndim, writable, dtypeptr]
        )
    ...
    def nrt_meminfo_as_pyobject(self, miptr):
        ...
        fn = mod.get_or_insert_function(fnty, name="NRT_meminfo_as_pyobject",)
        fn.return_value.add_attribute("noalias")
    ...
    def nrt_meminfo_from_pyobject(self, miobj):
        ...
        fn = mod.get_or_insert_function(fnty, name="NRT_meminfo_from_pyobject",)
        fn.return_value.add_attribute("noalias")
    ...
    def nrt_adapt_buffer_from_python(self, buf, ptr):
        assert self.context.enable_nrt
        fnty = Type.function(
            Type.void(), [Type.pointer(self.py_buffer_t), self.voidptr]
        )
    ...
    def alloca_buffer(self):
        ...
        ptr = cgutils.alloca_once_value(
            self.builder, lc.Constant.null(self.py_buffer_t)
        )
        return ptr
    ...
    def numba_buffer_adaptor(self, buf, ptr):
        fnty = Type.function(
            Type.void(), [ir.PointerType(self.py_buffer_t), self.voidptr]
        )
    ...
    def get_buffer(self, obj, pbuf):
        fnty = Type.function(Type.int(), [self.pyobj, ir.PointerType(self.py_buffer_t)])
    ...
    def recreate_record(self, pdata, size, dtype, env_manager):
        fnty = Type.function(
            self.pyobj, [Type.pointer(Type.int(8)), Type.int(), self.pyobj]
        )
