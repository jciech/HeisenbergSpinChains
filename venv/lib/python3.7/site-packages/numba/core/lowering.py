...
class Environment(_dynfunc.Environment):
    ...
    def __reduce__(self):
        return _rebuild_env, (self.globals["__name__"], self.consts, self.env_name,)
...
class BaseLower(object):
    ...
    def __init__(self, context, library, fndesc, func_ir, metadata=None):
        ...
        self.debuginfo = dibuildercls(module=self.module, filepath=func_ir.loc.filename)
...
class Lower(BaseLower):
    ...
    def pre_block(self, block):
        ...
        for call in block.find_exprs(op="call"):
            defn = ir_utils.guard(ir_utils.get_definition, self.func_ir, call.func,)
    ...
    def lower_yield(self, retty, inst):
        ...
        yret = self.context.cast(self.builder, val, typ, self.gentype.yield_type)
    ...
    def lower_binop(self, resty, expr, op):
        ...
        res = try_static_impl((_lit_or_omitted(static_lhs), rty), (static_lhs, rhs),)
        if res is not None:
            return cast_result(res)

        res = try_static_impl((lty, _lit_or_omitted(static_rhs)), (lhs, static_rhs),)
        if res is not None:
            return cast_result(res)
    ...
    def _lower_call_ObjModeDispatcher(self, fnty, expr, signature):
        ...
        callee = self.context.add_dynamic_addr(
            self.builder, id(entry_pt), info="with_objectmode",
        )
        ...
        with self.builder.if_else(has_exception) as (then, orelse):
            ...
            with orelse:
                ...
                native = self.pyapi.to_native_value(
                    fnty.dispatcher.output_types, ret_obj,
                )
                output = native.value
        ...
        with self.builder.if_else(
            ...
            with then:
                ...
                addr1 = lower_get_wrapper_address(
                    self.context, self.builder, pyaddr, sig, failure_mode="ignore"
                )
    ...
    def _lower_call_normal(self, fnty, expr, signature):
        ...
        if isinstance(expr.func, ir.Intrinsic) or isinstance(
            fnty, types.ObjModeDispatcher
        ):
    ...
    def lower_expr(self, resty, expr):
        if expr.op == "binop":
            ...
            signature = typing.signature(
                resty, self.typeof(expr.value.name), _lit_or_omitted(expr.index),
            )
            try:
            ...
            signature = typing.signature(
                resty, self.typeof(expr.value.name), self.typeof(expr.index.name),
            )
    ...
    def alloca_lltype(self, name, lltype):
        ...
        if is_uservar:
            ...
            self.debuginfo.mark_variable(
                self.builder, aptr, name=name, lltype=lltype, size=sizeof, loc=self.loc
            )
        return aptr
