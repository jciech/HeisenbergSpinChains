...
def overload(func, jit_options={}, strict=True, inline="never"):
    ...
    * a function that takes two arguments, both of which are instances of a
    ...
    def decorate(overload_func):
        template = make_overload_template(func, overload_func, opts, strict, inline)
        infer(template)
        if callable(func):
            infer_global(func, types.Function(template))
...
class _Intrinsic(object):
    ...
    def __reduce__(self):
        ...
        return (
            ...
            (self.__class__, str(self._uuid), self._name, reduce_func(self._defn)),
        )
