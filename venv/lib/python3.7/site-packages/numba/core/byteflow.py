...
_NO_RAISE_OPS = frozenset({"LOAD_CONST",})
...
class TraceRunner(object):
    ...
    def op_SETUP_WITH(self, state, inst):
        ...
        state.push_block(
            state.make_block(kind="WITH_FINALLY", end=inst.get_jump_target(),)
        )
    ...
    def _setup_try(self, kind, state, next, end):
        ...
        state.fork(
            ...
            extra_block=state.make_block(
                kind="TRY", end=end, reset_stack=False, handler=handler_block,
            ),
        )
...
class State(object):
    ...
    def make_temp(self, prefix=""):
        if not prefix:
            ...
            name = "${prefix}{offset}.{tempct}".format(
                prefix=prefix, offset=self._pc, tempct=len(self._temp_registers),
            )
    ...
    def fork(self, pc, npop=0, npush=0, extra_block=None):
        ...
        self._outedges.append(
            Edge(pc=pc, stack=tuple(stack), npush=npush, blockstack=tuple(blockstack),)
        )
        self.terminate()
