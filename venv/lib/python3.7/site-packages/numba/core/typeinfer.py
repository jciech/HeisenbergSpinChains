    ...
    def __call__(self, typeinfer):
        with new_error_context("typing of dict at {0}", self.loc):
            ...
            if not tsets:
                typeinfer.add_type(
                    self.target,
                    types.DictType(types.undefined, types.undefined),
                    loc=self.loc,
                )
            else:
                key_type, value_type = tsets[0]
                typeinfer.add_type(
                    self.target, types.DictType(key_type, value_type), loc=self.loc
                )
            ...
            for tp in typevars[self.iterator.name].get():
                ...
                if isinstance(tp, types.BaseTuple):
                    if len(tp) == self.count:
                        ...
                        raise ValueError(
                            "wrong tuple length for %r: expected %d, got %d"
...
class SetItemRefinement(object):
    ...
    def _refine_target_type(self, typeinfer, targetty, idxty, valty, sig):
        ...
        if isinstance(targetty, types.DictType) and not targetty.is_precise():
            ...
            typeinfer.add_type(
                self.target.name, refined, loc=self.loc,
            )
...
"""
    ...
    def get_generator_type(self, typdict, retty, raise_errors=True):
        ...
        if yield_type is None or isinstance(yield_type, types.Optional):
            ...
            if raise_errors:
                raise TypingError(
                    "Can't unify yield type from the following types: %s"
    ...
    def _unify_return_types(self, rettypes):
        if rettypes:
            ...
            if unified is None or not unified.is_precise():
                ...
                raise TypingError(
                    "Can't unify return type from the following types: %s"
    ...
    def sentry_modified_builtin(self, inst, gvar):
        ...
        """
        if (
            gvar.name in ("range", "xrange")
            and gvar.value not in utils.RANGE_ITER_OBJECTS
        ):
            bad = True
    ...
    def resolve_call(self, fnty, pos_args, kw_args):
        ...
        if isinstance(fnty, types.RecursiveCall) and not self._skip_recursion:
            ...
            if frame is None:
                ...
                fnty.overloads[args] = qualifying_prefix(
                    fndesc.modname, fndesc.unique_name
                )
                return sig

            fnid = frame.func_id
            fnty.overloads[args] = qualifying_prefix(fnid.modname, fnid.unique_name)
    ...
    def typeof_expr(self, inst, target, expr):
        if expr.op == "call":
            ...
            constraint = BuildMapConstraint(target.name, items=expr.items, loc=inst.loc)
            self.constraints.append(constraint)
        elif expr.op == "cast":
            self.constraints.append(
