...
def _define_nrt_decref(module, atomic_decr):
    ...
    """
    fn_decref = module.get_or_insert_function(incref_decref_ty, name="NRT_decref")
    # Cannot inline this for refcount pruning to work
    fn_decref.attributes.add("noinline")
    calldtor = module.add_function(
        ir.FunctionType(ir.VoidType(), [_pointer_type]), name="NRT_MemInfo_call_dtor"
    )
    ...
    newrefct = builder.call(
        atomic_decr, [builder.bitcast(ptr, atomic_decr.args[0].type)]
    )
...
def _define_atomic_cas(module, ordering):
    ...
    ftype = ir.FunctionType(
        ir.IntType(32),
        [_word_type.as_pointer(), _word_type, _word_type, _word_type.as_pointer()],
    )
