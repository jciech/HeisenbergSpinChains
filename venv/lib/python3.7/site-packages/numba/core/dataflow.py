...
class DataFlowAnalysis(object):
    ...
    def run_on_block(self, blk):
        ...
        for ib, pops in self.cfa.incoming_blocks(blk):
            ...
            if new_offset < 0:
                raise RuntimeError("computed negative stack offset for %s" % blk)
            if info.stack_offset is None:
                info.stack_offset = new_offset
            elif info.stack_offset != new_offset:
                warnings.warn("inconsistent stack offset for %s" % blk, RuntimeWarning)
            ...
            if info.syntax_blocks is None:
                ...
                warnings.warn(
                    "inconsistent entry syntax blocks for %s" % blk, RuntimeWarning
                )
    ...
    def handle_unknown_opcode(self, info, inst):
        raise UnsupportedError(
            ...
            loc=Loc(filename=self.bytecode.func_id.filename, line=inst.lineno),
        )
    ...
    def op_PRINT_ITEM(self, info, inst):
        warnings.warn(
            "Python2 style print partially supported.  Please use Python3 style print.",
            RuntimeWarning,
        )
        item = info.pop()
    ...
    def op_BUILD_SLICE(self, info, inst):
        ...
        slicevar = info.make_temp()
        res = info.make_temp()
        info.append(inst, start=start, stop=stop, step=step, res=res, slicevar=slicevar)
        info.push(res)
