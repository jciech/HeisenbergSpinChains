...
def _loop_lift_prepare_loop_func(loopinfo, blocks):
    ...
    blocks[loopinfo.returnto] = ir_utils.fill_callee_epilogue(
        block=ir.Block(scope=scope, loc=loc), outputs=loopinfo.outputs,
    )
    ...
    def get_ctxmgr_obj(var_ref):
        ...
        if ctxobj is ir.UNDEFINED:
            raise errors.CompilerError(
                "Undefined variable used as context manager", loc=blocks[blk_start].loc,
            )
    ...
    for stmt in blocks[blk_start].body:
        if isinstance(stmt, ir.EnterWith):
            ...
            if not hasattr(ctxobj, "mutate_with_body"):
                raise errors.CompilerError(
                    "Unsupported context manager in use", loc=blocks[blk_start].loc,
                )
            return ctxobj, extra
    # No contextmanager found?
    raise errors.CompilerError(
        "malformed with-context usage", loc=blocks[blk_start].loc,
    )
...
def _legalize_with_head(blk):
    ...
    if counters.pop(ir.EnterWith) != 1:
        raise errors.CompilerError(
            "with's head-block must have exactly 1 ENTER_WITH", loc=blk.loc,
        )
    if counters.pop(ir.Jump) != 1:
        raise errors.CompilerError(
            "with's head-block must have exactly 1 JUMP", loc=blk.loc,
        )
    ...
    if counters:
        raise errors.CompilerError(
            "illegal statements in with's head-block", loc=blk.loc,
        )
...
def _cfg_nodes_in_region(cfg, region_begin, region_end):
    ...
    while stack:
        ...
        nodes = set(
            [node for node in succs if node not in region_nodes and node != region_end]
        )
        stack.extend(nodes)
        region_nodes |= nodes
