...
def dead_code_elimination(func_ir, typemap=None, alias_map=None, arg_aliases=None):
    ...
    do_post_proc = False
    while remove_dead(
        func_ir.blocks, func_ir.arg_names, func_ir, typemap, alias_map, arg_aliases
    ):
        do_post_proc = True
    ...
    while True:
        if isinstance(callee_def, (ir.Global, ir.FreeVar)):
            ...
            if hasattr(def_val, "__module__"):
                ...
                if hasattr(numpy, value) and def_val == getattr(numpy, value):
                    attrs += ["numpy"]
...
def compile_to_numba_ir(
    mk_func, glbls, typingctx=None, arg_typs=None, typemap=None, calltypes=None
):
    """
