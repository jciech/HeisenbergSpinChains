...
_unsupported = set(
    [
        "frexp",  # this one is tricky, as it has 2 returns
...
class NdExpandDims(CallableTemplate):
    def generic(self):
        def typer(a, axis):
            if not isinstance(a, types.Array) or not isinstance(axis, types.Integer):
                return
...
def _homogeneous_dims(context, func_name, arrays):
    ...
    for a in arrays:
        if a.ndim != ndim:
            raise TypeError(
                "%s(): all the input arrays must have same number of dimensions"
                % func_name
            )
    return ndim
...
class NdConcatenate(CallableTemplate):
    def generic(self):
        def typer(arrays, axis=None):
            ...
            dtype, ndim = _sequence_of_arrays(self.context, "np.concatenate", arrays)
            if ndim == 0:
...
class NdStack(CallableTemplate):
    def generic(self):
        def typer(arrays, axis=None):
            ...
            dtype, ndim = _sequence_of_arrays(self.context, "np.stack", arrays)
    ...
    def generic(self):
        def typer(arrays):
            dtype, ndim = _sequence_of_arrays(
                self.context, "np.column_stack", arrays, dim_chooser=_column_stack_dims
            )
...
class VDot(CallableTemplate):
    def generic(self):
        def typer(a, b):
            ...
            if not isinstance(a.dtype, (types.Float, types.Complex)):
                raise TypingError(
                    "np.vdot() only supported on float and complex arrays"
                )
            return a.dtype
...
class Round(AbstractTemplate):
    def generic(self, args, kws):
        ...
        if (
            isinstance(arg, types.Array)
            and isinstance(arg.dtype, supported_scalars)
            and isinstance(out, types.Array)
            and isinstance(out.dtype, supported_scalars)
            and out.ndim == arg.ndim
        ):
            # arg can only be complex if out is complex too
            if not isinstance(arg.dtype, types.Complex) or isinstance(
                out.dtype, types.Complex
            ):
...
class Sinc(AbstractTemplate):
    def generic(self, args, kws):
        ...
        if isinstance(arg, supported_scalars) or (
            isinstance(arg, types.Array) and isinstance(arg.dtype, supported_scalars)
        ):
    ...
    def generic(self):
        ...
        def typer(ptr, shape, dtype=types.none):
            if ptr is types.voidptr:
                ...
                raise TypeError(
                    "%s(): pointer argument expected, got '%s'" % (func_name, ptr)
                )
            ...
            if dtype is types.none:
                ...
                raise TypeError("%s(): invalid dtype spec '%s'" % (func_name, dtype))

            ndim = _parse_shape(shape)
            if ndim is None:
                raise TypeError("%s(): invalid shape '%s'" % (func_name, shape))
