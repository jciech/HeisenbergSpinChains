...
def normalize_shape(shape):
    if isinstance(shape, types.Integer):
        ...
        isinstance(shape, types.BaseTuple) and all(isinstance(v, types.Integer))
        for v in shape
    ):
        ndim = len(shape)
...
class RandomTemplate(CallableTemplate):
    ...
    def array_typer(self, scalar_typer, size=None):
        ...
        if size is None:
            ...
            def typer(*args, **kwargs):
                if prefix == "random":
                    raise TypeError("unexpected size parameter for %r" % (self.key,))
                ...
                if sig is not None:
                    return signature(
                        types.Array(sig.return_type, ndim, "C"), *(sig.args + (shape,))
                    )
...
if numpy_version >= (1, 17):
    ...
    infer_global(np.random.sample, typing_key="np.random.sample",)(Random_random)
    infer_global(np.random.ranf, typing_key="np.random.ranf",)(Random_random)
...
@infer_global(np.random.negative_binomial, typing_key="np.random.negative_binomial")
...
@infer_global(np.random.standard_cauchy, typing_key="np.random.standard_cauchy")
@infer_global(np.random.standard_normal, typing_key="np.random.standard_normal")
@infer_global(
    np.random.standard_exponential, typing_key="np.random.standard_exponential"
)
