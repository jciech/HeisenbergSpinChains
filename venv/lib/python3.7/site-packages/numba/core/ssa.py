...
class _FixSSAVars(_BaseHandler):
    ...
    def on_assign(self, states, assign):
        ...
        if isinstance(rhs, ir.Inst):
            newdef = self._fix_var(states, assign, assign.value.list_vars(),)
            ...
            if newdef is not None and states["varname"] != newdef.target.name:
                return ir.Assign(
                    target=assign.target, value=newdef.target, loc=assign.loc,
                )

        return assign

    def on_other(self, states, stmt):
        newdef = self._fix_var(states, stmt, stmt.list_vars(),)
        if newdef is not None and states["varname"] != newdef.target.name:
            replmap = {states["varname"]: newdef.target}
            stmt = copy(stmt)
    ...
    def _find_def_from_top(self, states, label, loc):
        ...
        for deflabel, defstmt in defmap.items():
            ...
            if label in df:
                ...
                # insert phi
                phinode = ir.Assign(
                    target=freshvar, value=ir.Expr.phi(loc=loc), loc=loc,
                )
