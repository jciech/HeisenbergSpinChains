    ...
    def _fill_arrays(self):
        ...
        for i, arg in enumerate(self.args):
            if isinstance(arg, np.ndarray):
                ...
                raise TypeError(
                    "argument #%d has invalid type of %s" % (i + 1, type(arg))
                )
...
class GenerializedUFunc(object):
    ...
    def __call__(self, *args, **kws):
        ...
        newparams, newretval = self._broadcast(
            schedule, callsteps.kernel_parameters, callsteps.kernel_returnvalue
        )
...
class GUFuncCallSteps(object):
    ...
    def __init__(self, nin, nout, args, kwargs):
        ...
        self._need_device_conversion = (
            not any(self._is_device_array) and not user_output_is_device
        )
    ...
    def allocate_outputs(self, schedule, outdtype):
        ...
        if self._need_device_conversion or self.output is None:
            retval = self.device_array(shape=schedule.output_shapes[0], dtype=outdtype)
        else:
            retval = self.output
