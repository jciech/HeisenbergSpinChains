...
from numba.np.numpy_support import as_dtype, carray, farray, is_contiguous, is_fortran
...
from numba.core.extending import register_jitable, overload, overload_method, intrinsic
...
def set_range_metadata(builder, load, lower_bound, upper_bound):
    ...
    """
    range_operands = [
        Constant.int(load.type, lower_bound),
...
def populate_array(array, data, shape, strides, itemsize, meminfo, parent=None):
    ...
    *shape* and *strides* can be Python tuples or LLVM arrays.
    ...
    if meminfo is None:
        meminfo = Constant.null(context.get_value_type(datamodel.get_type("meminfo")))
    ...
    attrs = dict(
        shape=shape, strides=strides, data=data, itemsize=itemsize, meminfo=meminfo,
    )

    # Set `parent` attribute
    if parent is None:
        attrs["parent"] = Constant.null(
            context.get_value_type(datamodel.get_type("parent"))
        )
    else:
        attrs["parent"] = parent
    ...
    if isinstance(srcty, types.Buffer):
        ...
        def src_getitem(source_indices):
            ...
            getitem_impl = context.get_function(
                operator.getitem, signature(src_dtype, srcty, types.intp),
            )
...
def np_repeat(a, repeats):
    ...
    # type checking
    if isinstance(
        a, (types.Array, types.List, types.BaseTuple, types.Number, types.Boolean,)
    ):
...
def numpy_ones_nd(context, builder, sig, args):
    ...
    res = context.compile_internal(builder, ones, sig, args, locals={"c": valty})
    return impl_ret_new_ref(context, builder, sig.return_type, res)
...
@lower_builtin(np.linspace, types.Number, types.Number, types.Integer)
    ...
    try:
        ...
        if kind == "quicksort":
            sort = quicksort.make_jit_quicksort(
                lt=lt_floats if is_float else None, is_argsort=is_argsort
            )
...
def array_sort(context, builder, sig, args):
    ...
    sort_func = get_sort_func(
        kind="quicksort", is_float=isinstance(arytype.dtype, types.Float)
    )
