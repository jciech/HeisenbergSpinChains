...
class TestMath(unittest.TestCase):
    ...
    def _generic_test_unary(
        ...
        for dtype in types:
            ...
            np.testing.assert_allclose(
                ...
                err_msg="{0} ({1})".format(math_fn.__name__, dtype.__name__),
            )

    def _generic_test_binary(
        self,
        math_fn,
        npy_fn,
        cases=None,
    ...
    def test_trig(self):
        ...
        for fn in funcs:
            self._generic_test_unary(fn, getattr(np, fn.__name__), span=(-np.pi, np.pi))

    def test_trig_inv(self):
        funcs = [(math.asin, np.arcsin), (math.acos, np.arccos), (math.atan, np.arctan)]
    ...
    def test_trigh(self):
        ...
        for fn in funcs:
            self._generic_test_unary(fn, getattr(np, fn.__name__), span=(-4.0, 4.0))

    def test_trigh_inv(self):
        funcs = [
            (math.asinh, np.arcsinh, (-4, 4)),
    ...
    def test_classify(self):
        ...
        cases = (float("nan"), float("inf"), float("-inf"), float("-nan"), 0, 3, -2)
        for fn in funcs:
            self._generic_test_unary(fn, getattr(np, fn.__name__), cases=cases)
    ...
    def test_floor_ceil(self):
        ...
        for fn in funcs:
            ...
            self._generic_test_unary(
                fn, getattr(np, fn.__name__), span=(-1013.14, 843.21)
            )
            # cases that include "exact" integers
            self._generic_test_unary(
                fn, getattr(np, fn.__name__), span=(-16, 16), count=129
            )

    def test_fabs(self):
        funcs = [math.fabs]
        for fn in funcs:
            self._generic_test_unary(fn, getattr(np, fn.__name__), span=(-63.3, 63.3))

    def test_unary_exp(self):
        funcs = [math.exp]
        for fn in funcs:
            self._generic_test_unary(fn, getattr(np, fn.__name__), span=(-30, 30))
    ...
    def test_sqrt(self):
        funcs = [math.sqrt]
        for fn in funcs:
            self._generic_test_unary(fn, getattr(np, fn.__name__), span=(0, 1000))

    def test_log(self):
        funcs = [math.log, math.log10, math.log1p]
        for fn in funcs:
            self._generic_test_unary(fn, getattr(np, fn.__name__), span=(0.1, 2500))
