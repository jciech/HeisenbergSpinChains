...
def get_limiting_factors(group_size, vgpr_per_workitem, sgpr_per_wave):
    ...
    allowed_wave = min(
        allowed_wave_due_to_sgpr, max_wave_count, allowed_wave_due_to_vgpr
    )
    allowed_vgpr_per_workitem = _ceil(
        vgpr_per_simd / required_wave_count_per_simd / wave_size
    )
    # reasons
    reasons = set()
    ...
    # occupancy
    inflight_wave_per_cu = 0 if reasons else required_wave_count_per_simd * simd_per_cu
    occupancy = inflight_wave_per_cu / max_inflight_wave_per_cu

    return _limits(
        allowed_wave_due_to_sgpr=allowed_wave_due_to_sgpr,
...
_suggestions["group_size"] = "* Exceeds max group size (256)."
