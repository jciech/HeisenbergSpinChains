...
def _find_driver():
    ...
    if envpath is not None:
        try:
            ...
            raise HsaSupportError("NUMBA_HSA_DRIVER %s is not a valid path" % envpath)
        if not os.path.isfile(envpath):
            raise HsaSupportError(
                "NUMBA_HSA_DRIVER %s is not a valid file "
                "path.  Note it must be a filepath of the .so/"
                ".dll/.dylib or the driver"
                % envpath
            )
        candidates = [envpath]
    else:
...
class Driver(object):
    ...
    _hsa_properties = {
        ...
        "timestamp_frequency": (
            enums.HSA_SYSTEM_INFO_TIMESTAMP_FREQUENCY,
            ctypes.c_uint16,
        ),
    ...
    def create_program(
        ...
        self.hsa_ext_program_create(
            model, profile, rounding_mode, options, ctypes.byref(program)
        )
    ...
    def create_signal(self, initial_value, consumers=None):
        ...
        self.hsa_signal_create(
            initial_value, consumers_len, consumers, ctypes.byref(result)
        )
    ...
    def components(self):
        ...
        """
        return list(filter(lambda a: a.is_component, reversed(sorted(self.agents))))
...
class HsaWrapper(object):
    ...
    def __dir__(self):
        return sorted(
            set(dir(type(self)) + self.__dict__.keys() + self._hsa_properties.keys())
        )
...
class Agent(HsaWrapper):
    ...
    _hsa_properties = {
        ...
        "workgroup_max_dim": (
            enums.HSA_AGENT_INFO_WORKGROUP_MAX_DIM,
            ctypes.c_uint16 * 3,
        ),
    ...
    def _initialize_regions(self):
        ...
        self._regions = _RegionList(
            [MemRegion.instance_for(self, region_id) for region_id in region_ids]
        )
    ...
    def _initialize_mempools(self):
        ...
        self._mempools = _RegionList(
            [MemPool.instance_for(self, mempool_id) for mempool_id in mempool_ids]
        )
    ...
    def _create_queue(
        ...
        group_segment_size = (
            ctypes.c_uint32(-1) if group_segment_size is None else group_segment_size
        )
    ...
    def __repr__(self):
        return "<HSA agent ({0}): {1} {2} '{3}'{4}>".format(
            self._id,
            self.device,
            self.vendor_name,
            self.name,
...
class MemPool(HsaWrapper):
    ...
    _hsa_properties = {
        ...
        "_flags": (enums_ext.HSA_AMD_MEMORY_POOL_INFO_GLOBAL_FLAGS, ctypes.c_uint32),
        "size": (enums_ext.HSA_AMD_MEMORY_POOL_INFO_SIZE, ctypes.c_size_t),
        "alloc_allowed": (
            enums_ext.HSA_AMD_MEMORY_POOL_INFO_RUNTIME_ALLOC_ALLOWED,
            ctypes.c_bool,
        ),
        "alloc_granule": (
            enums_ext.HSA_AMD_MEMORY_POOL_INFO_RUNTIME_ALLOC_GRANULE,
            ctypes.c_size_t,
        ),
        "alloc_alignment": (
            enums_ext.HSA_AMD_MEMORY_POOL_INFO_RUNTIME_ALLOC_ALIGNMENT,
            ctypes.c_size_t,
        ),
        "accessible_by_all": (
...
class MemRegion(HsaWrapper):
    ...
    _hsa_properties = {
        "segment": (enums.HSA_REGION_INFO_SEGMENT, drvapi.hsa_region_segment_t),
        "_flags": (enums.HSA_REGION_INFO_GLOBAL_FLAGS, drvapi.hsa_region_global_flag_t),
        "host_accessible": (
            enums_ext.HSA_AMD_REGION_INFO_HOST_ACCESSIBLE,
            ctypes.c_bool,
        ),
        "size": (enums.HSA_REGION_INFO_SIZE, ctypes.c_size_t),
        "alloc_max_size": (enums.HSA_REGION_INFO_ALLOC_MAX_SIZE, ctypes.c_size_t),
        "alloc_alignment": (
            enums.HSA_REGION_INFO_RUNTIME_ALLOC_ALIGNMENT,
            ctypes.c_size_t,
        ),
        "alloc_granule": (enums.HSA_REGION_INFO_RUNTIME_ALLOC_GRANULE, ctypes.c_size_t),
        "alloc_allowed": (enums.HSA_REGION_INFO_RUNTIME_ALLOC_ALLOWED, ctypes.c_bool),
    }
...
class Queue(object):
    ...
    def _get_packet(self, packet_type):
        ...
        assert ctypes.sizeof(packet_type) == ctypes.sizeof(
            drvapi.hsa_kernel_dispatch_packet_t
        )
    ...
    def insert_barrier(self, dep_signal):
        with self._get_packet(drvapi.hsa_barrier_and_packet_t) as packet:
            ...
            header = 0
            header |= (
                enums.HSA_FENCE_SCOPE_SYSTEM
                << enums.HSA_PACKET_HEADER_ACQUIRE_FENCE_SCOPE
            )
            header |= (
                enums.HSA_FENCE_SCOPE_SYSTEM
                << enums.HSA_PACKET_HEADER_RELEASE_FENCE_SCOPE
            )
    ...
    def dispatch(
        self, symbol, kernargs, workgroup_size=None, grid_size=None, signal=None
    ):
        ...
        if workgroup_size > tuple(self._agent.workgroup_max_dim)[:dims]:
            ...
            raise HsaDriverError(
                msg.format(workgroup_size, tuple(self._agent.workgroup_max_dim)[:dims])
            )
        ...
        with self._get_packet(drvapi.hsa_kernel_dispatch_packet_t) as packet:
            ...
            packet.kernarg_address = 0 if kernargs is None else kernargs.value
            ...
            header |= (
                enums.HSA_PACKET_TYPE_KERNEL_DISPATCH << enums.HSA_PACKET_HEADER_TYPE
            )
    ...
    def __dir__(self):
        return sorted(set(dir(self._id.contents) + self.__dict__.keys()))
...
class Signal(object):
    ...
    def wait_until_ne_one(self, timeout=None):
        ...
        hsa.hsa_signal_wait_acquire(
            self._id,
            enums.HSA_SIGNAL_CONDITION_NE,
            one,
            expire,
...
class BrigModule(object):
    def __init__(self, brig_buffer):
        ...
        self._buffer = buf
        self._id = ctypes.cast(ctypes.addressof(buf), drvapi.hsa_ext_module_t)
    ...
    def __repr__(self):
        return "<BrigModule id={0} size={1}bytes>".format(hex(id(self)), len(self))
...
class Program(object):
    def __init__(
        ...
        hsa.hsa_system_extension_supported(
            enums.HSA_EXTENSION_FINALIZER,
            version_major,
            version_minor,
        ...
        # populate struct
        hsa.hsa_system_get_extension_table(
            enums.HSA_EXTENSION_FINALIZER,
            version_major,
            version_minor,
        ...
        ret = self._ftabl.hsa_ext_program_create(
            model, profile, rounding_mode, options, ctypes.byref(self._id)
        )

        check_fptr_return(ret)

        self._as_parameter_ = self._id
        weakref.finalize(self, self._ftabl.hsa_ext_program_destroy, self._id)
    ...
    def finalize(self, isa, callconv=0, options=None):
        ...
        ctypes.memset(
            ctypes.byref(control_directives), 0, ctypes.sizeof(control_directives)
        )
        self._ftabl.hsa_ext_program_finalize(
            self._id,
            isa,
            callconv,
            control_directives,
...
class Executable(object):
    def __init__(self):
        ...
        hsa.hsa_executable_create(
            enums.HSA_PROFILE_FULL,
    ...
    def load(self, agent, code_object):
        hsa.hsa_executable_load_code_object(self._id, agent._id, code_object._id, None)
...
class MemoryPointer(object):
    ...
    def allow_access_to(self, *agents):
        ...
        hsa.hsa_amd_agents_allow_access(ct, agent_array, None, self.device_pointer)
...
class Context(object):
    ...
    def memalloc(self, nbytes, memTypeFlags=None, hostAccessible=True):
        ...
        for region_id in regions:
            try:
                mem = MemRegion.instance_for(self._agent, region_id).allocate(nbytes)
        ...
        if mem is None:
            raise RuntimeError(
                "Memory allocation failed. No agent/region               combination"
                " could meet allocation restraints               (hardware = %s, size ="
                " %s, flags = %s)."
                % (hw, nbytes, memTypeFlags)
            )

        fin = _make_mem_finalizer(hsa.hsa_memory_free)
        ret = MemoryPointer(weakref.proxy(self), mem, nbytes, finalizer=fin(self, mem))
    ...
    def mempoolalloc(self, nbytes, allow_access_to=(), finegrain=False):
        ...
        *finegrain*
        """
        mempool = self.finegrain_mempool if finegrain else self.coarsegrain_mempool

        buff = mempool.allocate(nbytes)
        fin = _make_mem_finalizer(hsa.hsa_amd_memory_pool_free)
        mp = MemoryPointer(weakref.proxy(self), buff, nbytes, finalizer=fin(self, buff))
        mp.allow_access_to(*allow_access_to)
        self.allocations[buff.value] = mp
        return mp.own()

    def memhostalloc(self, size, finegrain, allow_access_to):
        mem = self.mempoolalloc(
            size, allow_access_to=allow_access_to, finegrain=finegrain
        )
        return HostMemory(
            weakref.proxy(self), owner=mem, pointer=mem.device_pointer, size=mem.size
        )
...
def async_host_to_dGPU(dst_ctx, src_ctx, dst, src, size, stream):
    ...
    async_copy_dgpu(
        dst_ctx=dst_ctx,
        src_ctx=src_ctx,
        src=host_pointer(src),
        dst=device_pointer(dst),
        size=size,
        stream=stream,
    )
...
def async_copy_dgpu(dst_ctx, src_ctx, dst, src, size, stream):
    ...
    hsa.hsa_amd_memory_async_copy(
        dst, dst_ctx._agent._id, src, src_ctx._agent._id, size, *signals
    )
