...
def compile_kernel(pyfunc, args, debug=False):
    ...
    hsakern = HSAKernel(
        llvm_module=kernel.module, name=kernel.name, argtypes=cres.signature.args
    )
    return hsakern
...
_CacheEntry = namedtuple("_CachedEntry", ["symbol", "executable", "kernarg_region"])
    ...
    def get(self):
        ...
        if result is None:
            ...
            driver.hsa.hsa_code_object_deserialize(
                ctypes.addressof(bas), len(self._binary), None, ctypes.byref(code_ptr)
            )
            ...
            result = _CacheEntry(
                symbol=symobj, executable=ex, kernarg_region=kernarg_region
            )
...
class HSAKernel(HSAKernelBase):
    ...
    def __init__(self, llvm_module, name, argtypes):
        ...
        self._argloc = []
        # cached program
        self._cacheprog = _CachedProgram(entry_name=self.entry_name, binary=self.binary)
    ...
    def bind(self):
        ...
        if entry.symbol.kernarg_segment_size > 0:
            sz = ctypes.sizeof(ctypes.c_byte) * entry.symbol.kernarg_segment_size
