...
from .enums import CLK_LOCAL_MEM_FENCE, CLK_GLOBAL_MEM_FENCE
...
def _host_array(finegrain, shape, dtype, strides, order):
    ...
    buf = devices.get_cpu_context().memhostalloc(
        bytesize, finegrain=finegrain, allow_access_to=agents
    )
    arr = np.ndarray(shape=shape, strides=strides, dtype=dtype, order=order, buffer=buf)
...
def coarsegrain_array(shape, dtype=np.float, strides=None, order="C"):
    ...
    """
    return _host_array(
        finegrain=False, shape=shape, dtype=dtype, strides=strides, order=order
    )
