...
from numba.np.ufunc.deviceufunc import (
    UFuncMechanism,
    GenerializedUFunc,
    GUFuncCallSteps,
)
...
class HsaUFuncMechanism(UFuncMechanism):
    ...
    def broadcast_device(self, ary, shape):
        if dgpu_present:
            ...
            return np.ndarray(shape=shape, strides=strides, dtype=ary.dtype, buffer=ary)
    ...
    def _broadcast_scalar_input(self, ary, shape):
        if dgpu_present:
            return devicearray.DeviceNDArray(
                shape=shape, strides=(0,), dtype=ary.dtype, dgpu_data=ary.dgpu_data
            )
        else:
            return np.lib.stride_tricks.as_strided(ary, shape=(shape,), strides=(0,))
