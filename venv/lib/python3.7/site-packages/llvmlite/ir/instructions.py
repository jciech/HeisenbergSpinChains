...
class CallInstr(Instruction):
    def __init__(
        self, parent, func, args, name="", cconv=None, tail=False, fastmath=()
    ):
        self.cconv = (
    ...
    def _descr(self, buf, add_metadata):
        args = ", ".join(
            ["{0} {1}".format(a.type, a.get_reference()) for a in self.args]
        )
...
class SelectInstr(Instruction):
    def __init__(self, parent, cond, lhs, rhs, name=""):
        ...
        super(SelectInstr, self).__init__(
            parent, lhs.type, "select", [cond, lhs, rhs], name=name
        )
...
class LoadInstr(Instruction):
    def __init__(self, parent, ptr, name=""):
        super(LoadInstr, self).__init__(
            parent, ptr.type.pointee, "load", [ptr], name=name
        )
        self.align = None
...
class LoadAtomicInstr(Instruction):
    def __init__(self, parent, ptr, ordering, align, name=""):
        super(LoadAtomicInstr, self).__init__(
            parent, ptr.type.pointee, "load atomic", [ptr], name=name
        )
...
class PhiInstr(Instruction):
    ...
    def descr(self, buf):
        ...
        buf.append(
            "phi {0} {1} {2}\n".format(
                self.type, incs, self._stringify_metadata(leading_comma=True),
            )
        )
...
class InsertElement(Instruction):
    def __init__(self, parent, vector, value, index, name=""):
        ...
        if not value.type == vector.type.element:
            raise TypeError(
                "value needs to be of type % not %." % (vector.type.element, value.type)
            )
        if not isinstance(index.type, types.IntType):
            raise TypeError("index needs to be of IntType.")
        typ = vector.type
