    ...
    {  # Subroutine
        ...
        "declfortranroutine": {
            l_and(l_not(l_or(ismoduleroutine, isintent_c)), l_not(isdummyroutine)): (
                "extern void"
                " #F_FUNC#(#fortranname#,#FORTRANNAME#)(#callprotoargument#);"
            ),
        ...
        "routine_def": {
            l_not(l_or(ismoduleroutine, isintent_c, isdummyroutine)): (
                '\t{"#name#",-1,{{-1}},0,(char'
                " *)#F_FUNC#(#fortranname#,#FORTRANNAME#),(f2py_init_func)#apiname#,doc_#apiname#},"
            ),
            l_and(l_not(ismoduleroutine), isintent_c, l_not(isdummyroutine)): (
                '\t{"#name#",-1,{{-1}},0,(char'
                " *)#fortranname#,(f2py_init_func)#apiname#,doc_#apiname#},"
            ),
...
\t\t\t\t/*(*f2py_func)(#callfortran#);*/"""
    ...
    {  # Wrapped function
        ...
        "routine_def": {
            l_not(l_or(ismoduleroutine, isdummyroutine)): (
                '\t{"#name#",-1,{{-1}},0,(char'
                " *)#F_WRAPPEDFUNC#(#name_lower#,#NAME#),(f2py_init_func)#apiname#,doc_#apiname#},"
            ),
            isdummyroutine: (
                '\t{"#name#",-1,{{-1}},0,NULL,(f2py_init_func)#apiname#,doc_#apiname#},'
            ),
...
"""
            ...
            {
                l_and(debugcapi, l_not(iscomplexfunction)): (
                    '\tfprintf(stderr,"#routdebugshowvalue#\\n",#name#_return_value);'
                )
            },
        ],
        "pyobjfrom": {
            iscomplexfunction: (
                "\t#name#_return_value_capi = pyobj_from_#ctype#1(#name#_return_value);"
            )
        },
        "need": [
    ...
    {  # String function # in use for --no-wrap
        ...
        "callfortranroutine": [
            ...
            "\tif ((#name#_return_value ="
            " (string)malloc(sizeof(char)*(#name#_return_value_len+1))) == NULL) {",
...
""",
    ...
    {  # Debugging
        "routdebugenter": (
            '\tfprintf(stderr,"debug-capi:Python C/API function'
            ' #modulename#.#name#(#docsignature#)\\n");'
        ),
        "routdebugleave": (
            '\tfprintf(stderr,"debug-capi:Python C/API function #modulename#.#name#:'
            ' successful.\\n");'
        ),
        "routdebugfailure": (
            '\tfprintf(stderr,"debug-capi:Python C/API function #modulename#.#name#:'
...
arg_rules = [
    ...
    {  # Common
        ...
        "docstropt": {
            isintent_nothide: (
                "#varname#_extra_args : input tuple, optional\\n    Default: ()"
            )
        },
        "docstrcbs": "#cbdocstr#",
        ...
        "latexdocstropt": {
            isintent_nothide: (
                ...
                " arguments for call-back function {{}\\verb@#varname#@{}}."
            )
        },
        "decl": [
...
\tif (f2py_success) {"""
    ...
    },
    {
        "frompyobj": [
            ...
            {
                hasinitvalue: (
                    "\tif (#varname#_capi == Py_None) #varname# = #init#; else"
                ),
                "_depend": "",
            },
            {
...
\t\tif (f2py_success) {"""
    ...
    },
    {
        "frompyobj": [
            {
                hasinitvalue: (
                    "\tif (#varname#_capi==Py_None) {#varname#.r = #init.r#,"
                    " #varname#.i = #init.i#;} else"
                )
            },
            {
                l_and(isoptional, l_not(hasinitvalue)): (
            ...
            "\t\tf2py_success ="
            ' #ctype#_from_pyobj(&#varname#,#varname#_capi,"#pyname#() #nth#'
            " (#varname#) can't be converted to #ctype#\");\n\tif (f2py_success) {",
        ],
...
\tif (f2py_success) {"""
    ...
    {
        "frompyobj": (
            "\tcapi_#varname#_intent |= (capi_overwrite_#varname#?0:F2PY_INTENT_COPY);"
        ),
        "_check": l_and(isarray, isintent_overwrite),
        "_depend": "",
    },
    ...
    },
    {
        "frompyobj": [
            ...
            "\tcapi_#varname#_intent |= #intent#;",
            {
                isintent_hide: (
                    "\tcapi_#varname#_tmp ="
                    " array_from_pyobj(#atype#,#varname#_Dims,#varname#_Rank,capi_#varname#_intent,Py_None);"
                )
            },
            {
                isintent_nothide: (
                    "\tcapi_#varname#_tmp ="
                    " array_from_pyobj(#atype#,#varname#_Dims,#varname#_Rank,capi_#varname#_intent,#varname#_capi);"
                )
            },
            """\
...
def buildmodule(m, um):
    ...
    for n in m["interfaced"]:
        ...
        if not nb:
            errmess(
                'buildmodule: Could not found the body of interfaced routine "%s".'
                " Skipping.\n"
                % (n)
            )
            continue
