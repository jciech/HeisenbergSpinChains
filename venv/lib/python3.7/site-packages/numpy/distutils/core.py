    ...
    for libitem in libraries:
        if is_sequence(libitem):
            if is_sequence(item):
                if item[0] == libitem[0]:
                    if item[1] is libitem[1]:
                        return
                    warnings.warn(
                        "[0] libraries list contains %r with different build_info"
                        % (item[0],),
                        stacklevel=2,
                    )
                    break
                ...
                if item == libitem[0]:
                    warnings.warn(
                        "[1] libraries list contains %r with no build_info"
                        % (item[0],),
                        stacklevel=2,
                    )
                    break
            ...
            if is_sequence(item):
                if item[0] == libitem:
                    warnings.warn(
                        "[2] libraries list contains %r with no build_info"
                        % (item[0],),
                        stacklevel=2,
                    )
                    break
        ...
        if is_sequence(item):
            if item[0] == lib_name:
                if item[1] is build_info:
                    return
                warnings.warn(
                    "[3] libraries list contains %r with different build_info"
                    % (lib_name,),
                    stacklevel=2,
                )
                break
